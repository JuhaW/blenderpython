...
def bmaxAlign_Execute(self, ctx):
    ...
    if self.c_mode == '3DC':
        if self.pos_x == True:
            ctx.scene.cursor_location.x = a_box[t_id].x
        else:
            ctx.scene.cursor_location.x = self.pos_curs.x
        if self.pos_y == True:
            ctx.scene.cursor_location.y = a_box[t_id].y
        else:
            ctx.scene.cursor_location.y = self.pos_curs.y
        if self.pos_z == True:
            ctx.scene.cursor_location.z = a_box[t_id].z
        else:
    ...
    if self.t_mode == '3DC':
        ...
        for i, obj in enumerate(ctx.selected_objects):
            if self.pos_x == True:
                obj.location.x = scp.x + (self.pos_list[i][2].x - self.pos_list[i][c_id].x)
            else:
                obj.location.x = self.pos_list[i][2].x
            if self.pos_y == True:
                obj.location.y = scp.y + (self.pos_list[i][2].y - self.pos_list[i][c_id].y)
            else:
                obj.location.y = self.pos_list[i][2].y
            if self.pos_z == True:
                obj.location.z = scp.z + (self.pos_list[i][2].z - self.pos_list[i][c_id].z)
            else:
    ...
    for i, obj in enumerate(ctx.selected_objects):
        if obj != ctx.active_object:
            # position
            if self.pos_x == True:
                obj.location.x = a_box[t_id].x + (self.pos_list[i][2].x - self.pos_list[i][c_id].x)
            else:
                obj.location.x = self.pos_list[i][2].x
            if self.pos_y == True:
                obj.location.y = a_box[t_id].y + (self.pos_list[i][2].y - self.pos_list[i][c_id].y)
            else:
                obj.location.y = self.pos_list[i][2].y
            if self.pos_z == True:
                obj.location.z = a_box[t_id].z + (self.pos_list[i][2].z - self.pos_list[i][c_id].z)
            else:
                obj.location.z = self.pos_list[i][2].z
            # rotation
            if self.rot_x == True:
