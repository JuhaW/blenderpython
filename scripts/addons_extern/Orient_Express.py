...
class RotatePanel(bpy.types.Panel):
        ...
        v = [1, 2, 4]
        r = ['X', 'Y', 'Z']
        bpy.types.Scene.rotx = bpy.props.FloatProperty(default=90)
        bpy.types.Scene.roty = bpy.props.FloatProperty(default=90)
        bpy.types.Scene.rotz = bpy.props.FloatProperty(default=90)
        bpy.types.Scene.boolx = bpy.props.BoolProperty(default=True)
        bpy.types.Scene.booly = bpy.props.BoolProperty(default=True)
        bpy.types.Scene.boolz = bpy.props.BoolProperty(default=True)
...
class Orientation_create(bpy.types.Operator):
        """Select objects, Edit mode select geometry where to
        align transformation of the selected objects"""
...
class Object_create_empty(bpy.types.Operator):
        """Create an empty object at the location
...
class RotateX(bpy.types.Operator):
        """Rotate the components of a mesh around the local axis.
        If manipulator icon is enabled the original
        of the model is maintained."""
        bl_idname = "rotate.x"
        bl_label = "Rotate"

        axis = bpy.props.FloatVectorProperty()
        FILTER_OUT = ['EMPTY', 'ARMATURE', 'SPEAKER', 'LATTICE',
                      'CAMERA', 'LAMP', 'SURFACE', 'CURVE', 'FONT']
