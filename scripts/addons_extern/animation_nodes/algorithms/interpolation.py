...
def getInterpolationPreset(name="LINEAR", easeIn=True, easeOut=True):
    ...
    if name in exponentOfName.keys():
        if easeIn and easeOut:
            return assignArguments(powerInOut, exponentOfName[name])
        if easeIn:
            return assignArguments(powerIn, exponentOfName[name])
    ...
    if name == "BACK":
        if easeIn and easeOut:
            return assignArguments(backInOut, 1.7)
        if easeIn:
            return assignArguments(backIn, 1.7)
...
def bounceInOut(x, settings):
    if x <= 0.5:
        return (1 - bounceOut(1 - x * 2, settings)) / 2
    else:
        return bounceOut(x * 2 - 1, settings) / 2 + 0.5
