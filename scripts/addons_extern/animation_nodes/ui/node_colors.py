...
def colorNetwork(network, nodes):
    ...
    for node in nodes:
        if not node.useNetworkColor:
            continue
        node.use_custom_color = True
        color = networkColor
        if node.bl_idname == "an_InvokeSubprogramNode":
            if node.subprogramNetwork:
                color = getNetworkColor(node.subprogramNetwork)
        node.color = color
...
def getNetworkColor(network):
    ...
    if network.type == "Invalid":
        return colors.invalidNetwork
    if network.type == "Main":
        return colors.mainNetwork
...
def draw(self, context):
    ...
    if not getattr(node, "isAnimationNode", False):
        return
    ...
    if node.bl_idname == "an_InvokeSubprogramNode":
        network = node.subprogramNetwork
    else:
        network = node.network

    if getattr(network, "isSubnetwork", False):
        col.prop(network.ownerNode, "networkColor", text="")
    else:
        writeText(col, "Only subprograms have a custom network color", width=25)
