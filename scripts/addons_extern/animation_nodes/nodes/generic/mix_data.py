...
class MixDataNode(bpy.types.Node, AnimationNode):
    ...
    def getExecutionCode(self):
        lines = []
        if self.clampFactor:
            lines.append("f = min(max(factor, 0.0), 1.0)")
        else:
...
def getMixCode(dataType, mix1="a", mix2="b", factor="f", result="result"):
    if dataType in ("Float", "Vector", "Quaternion"):
        return "{} = {} * (1 - {}) + {} * {}".format(result, mix1, factor, mix2, factor)
    if dataType == "Matrix":
        return "{} = {}.lerp({}, {})".format(result, mix1, mix2, factor)
    if dataType == "Color":
        return "{} = [v1 * (1 - {}) + v2 * {} for v1, v2 in zip({}, {})]".format(result, factor, factor, mix1, mix2)
    if dataType == "Euler":
        return "{} = animation_nodes.utils.math.mixEulers({}, {}, {})".format(result, mix1, mix2, factor)
