...
class VectorMathNode(bpy.types.Node, AnimationNode):
    ...
    def getExecutionCode(self):
        ...
        if op == "ADD":
            ...
            yield "result = mathutils.Vector((a[0] * b[0], a[1] * b[1], a[2] * b[2]))"
        elif op == "CROSS":
            yield "result = a.cross(b)"
        elif op == "DIVIDE":
            ...
            yield "result = a.project(b)"
        elif op == "REFLECT":
            yield "result = a.reflect(b)"
        elif op == "NORMALIZE":
            yield "result = a.normalized() * scale"
        elif op == "SCALE":
            yield "result = a * scale"
        elif op == "ABSOLUTE":
            yield "result = mathutils.Vector((abs(a.x), abs(a.y), abs(a.z)))"
        elif op == "SNAP":
            yield "result = a.copy()"
