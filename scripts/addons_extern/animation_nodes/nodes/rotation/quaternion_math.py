...
class QuaternionMathNode(bpy.types.Node, AnimationNode):
    ...
    def getExecutionCode(self):
        ...
        if op == "ADD":
            ...
            return "result = a * b"
        elif op == "ROTATION_DIFFERENCE":
            return "result = a.rotation_difference(b)"
        elif op == "MULTIPLY":
            return "result = mathutils.Quaternion((A * B for A, B in zip(a, b)))"
        elif op == "DIVIDE":
            ...
            return "result = a.cross(b)"
        elif op == "NORMALIZE":
            return "result = a.normalized() * scale"
        elif op == "SCALE":
            return "result = a * scale"
        elif op == "ABSOLUTE":
            return "result = mathutils.Quaternion((abs(A) for A in a))"
        elif op == "INVERT":
            return "result = a.inverted()"
        elif op == "CONJUGATE":
            return "result = a.conjugated()"
        elif op == "SNAP":
