...
class ConstructInterpolationNode(bpy.types.Node, AnimationNode):
    ...
    def getExecutionCode(self):
        ...
        if not (self.easeIn or self.easeOut):
            return "interpolation = self.getLinear()"
        if c == "LINEAR":
            return "interpolation = self.getLinear()"
        if c == "SINUSOIDAL":
            return "interpolation = self.getSinusoidal()"
        if c == "POWER":
            return "interpolation = self.getPower(intExponent)"
        if c == "EXPONENTIAL":
            return "interpolation = self.getExponential(floatBase, intExponent)"
        if c == "CIRCULAR":
            return "interpolation = self.getCircular()"
        if c == "BACK":
            return "interpolation = self.getBack(floatSize)"
        if c == "BOUNCE":
            return "interpolation = self.getBounce(intBounces, floatSize)"
        if c == "ELASTIC":
            return "interpolation = self.getElastic(floatBase, floatExponent, intBounces)"
    ...
    def getPower(self, exponent):
        ...
        if self.easeIn and self.easeOut:
            return assignArguments(powerInOut, exponent)
        if self.easeIn:
            return assignArguments(powerIn, exponent)
    ...
    def getExponential(self, base, exponent):
        ...
        if self.easeIn and self.easeOut:
            return assignArguments(exponentialInOut, settings)
        if self.easeIn:
            return assignArguments(exponentialIn, settings)
    ...
    def getBack(self, back):
        if self.easeIn and self.easeOut:
            return assignArguments(backInOut, back)
        if self.easeIn:
            return assignArguments(backIn, back)
        return assignArguments(backOut, back)

    def getBounce(self, bounces, base):
        settings = prepareBounceSettings(bounces, base)
        if self.easeIn and self.easeOut:
            return assignArguments(bounceInOut, settings)
        if self.easeIn:
            return assignArguments(bounceIn, settings)
    ...
    def getElastic(self, base, exponent, bounces):
        ...
        if self.easeIn and self.easeOut:
            return assignArguments(elasticInOut, settings)
        if self.easeIn:
            return assignArguments(elasticIn, settings)
