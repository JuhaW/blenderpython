...
class NumberListMathNode(bpy.types.Node, AnimationNode):
    ...
    def getExecutionCode(self):
        if self.operation in ("ADD", "AVERAGE"):
            yield "result = functools.reduce(operator.add, numbers, 0)"
        if self.operation == "MULTIPLY":
            yield "result = functools.reduce(operator.mul, numbers, 1)"
        if self.operation == "MIN":
            yield "result = min(numbers) if len(numbers) > 0 else 0"
        if self.operation == "MAX":
            yield "result = max(numbers) if len(numbers) > 0 else 0"
        if self.operation == "AVERAGE":
            yield "if len(numbers) > 0: result /= len(numbers)"
