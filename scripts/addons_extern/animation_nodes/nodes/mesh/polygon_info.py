    ...
    def getExecutionCode(self):
        ...
        if self.copyVertices:
            if isLinked["vertexLocations"]:
                yield "vertexLocations = [v.copy() for v in polygon.vertexLocations]"
            if isLinked["center"]:
                yield "center = polygon.center.copy()"
            if isLinked["normal"]:
                yield "normal = polygon.normal.copy()"
        else:
            if isLinked["vertexLocations"]:
                yield "vertexLocations = polygon.vertexLocations"
            if isLinked["center"]:
                yield "center = polygon.center"
            if isLinked["normal"]:
                yield "normal = polygon.normal"

        if isLinked["area"]:
            yield "area = polygon.area"
        if isLinked["materialIndex"]:
            yield "materialIndex = polygon.materialIndex"
