...
class FCurveKeyframesNode(bpy.types.Node, AnimationNode):
    ...
    def getExecutionCode(self):
        ...
        if isLinked["keyframesFrames"]:
            yield "    keyframesFrames = [point.co[0] for point in fCurve.keyframe_points]"
        if isLinked["keyframesValues"]:
            yield "    keyframesValues = [point.co[1] for point in fCurve.keyframe_points]"
