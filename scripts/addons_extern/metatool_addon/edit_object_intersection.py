...
class CMesh:
    ...
    def neighbour(self, edgekey):
        ...
        if key in self.__neighbours:
            return self.__neighbours[key]
        else:
            return None
    ...
    def cpindex(self, cpoint):
        ...
        """
        global _cpoints
        if cpoint in _cpoints:
            return _cpoints.index(cpoint)
        else:
            return None
    ...
    def Intersect(self, key, face):
        ...
        if result:
            ...
            if len(result) == 4:  # I have never seen such case
                ...
                pass  # len in 1,2: nothing special
            ...
            if len(result) > 1:  # we will not append the single point to results
                ...
                if len(result) == 3:
                    cp = result[1]
                    if cp not in _cpoints:
                        _cpoints.insert(0, cp)
...
class CEdge:
    ...
    def Intersect(self, face):
        ...
        """
        if self.ray.length < EPSILON:
            return None  # this edge hardly exists!
...
class CPoint:
    ...
    def Assign(self, cpoint):
        ...
        if self.next is cpoint or self.prev is cpoint:
            return  # already done
        else:
            if not self.next:
                self.next = cpoint
    ...
    def GetOpposite(self, cpoint):
        ...
        """
        if cpoint is self.next:
            return self.prev
        elif cpoint is self.prev:
            return self.next
        else:
        ...
        if (not cp.IsTemporary() or allEdges) and not cp.BelongsTo(omit):
            ...
            if prev is None:  # Init: we are looking for the first valid point
                ...
                if r.length > EPSILON:  # if end and start point do not overlap:
                    ...
                    pass  # loop ends overlap, proceed to the next cp
