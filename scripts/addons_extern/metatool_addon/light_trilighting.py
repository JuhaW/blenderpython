...
class TriLighting_New(bpy.types.Operator):
    """TriL ightning"""
    bl_idname = "object.trilighting_new"        # unique identifier for buttons and menu items to reference.
...
def register():
    bpy.utils.register_class(TriLighting_New)
    bpy.types.VIEW3D_PT_tools_object.append(panel_func)

def unregister():
    bpy.utils.unregister_class(TriLighting_New)
