...
def AddNewVertexGroup(name):
    ...
    try:
        bpy.context.object.vertex_groups[groupName]
    except:
        return bpy.context.object.vertex_groups.new(name)
    return None
...
def GetGroupByName(name):
    try:
        return bpy.context.object.vertex_groups[name]
    except:
        return None
...
def init_handler(scene):
    try:
        bpy.app.handlers.scene_update_pre.remove(init_handler)
    except:
        return
    bpy.ops.wm.edger()
