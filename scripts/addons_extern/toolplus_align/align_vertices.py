...
class View3D_TP_Align_to_Normal(bpy.types.Operator):
    """Align selected Mesh to active Face in Normal Z Direction / depend on pivot point"""
...
class View3D_TP_GRP_Align_X(bpy.types.Operator):
    """Align Stroke to X Axis"""
    bl_idname = "tp_ops.grp_stroke_align_x"
    bl_label = "X Align"
    bl_options = {'REGISTER', 'UNDO'}
            
    def execute(self, context):  
        bpy.ops.transform.resize(value=(0, 1, 1), constraint_axis=(True, False, False), constraint_orientation='GLOBAL', mirror=False, proportional='DISABLED', proportional_edit_falloff='SMOOTH', proportional_size=1, gpencil_strokes=True)
        return {'FINISHED'}       

class View3D_TP_GRP_Align_Y(bpy.types.Operator):
    """Align Stroke to Y Axis"""
    bl_idname = "tp_ops.grp_stroke_align_y"
    bl_label = "Y Align"
    bl_options = {'REGISTER', 'UNDO'}
            
    def execute(self, context):  
        bpy.ops.transform.resize(value=(1, 0, 1), constraint_axis=(False, True, False), constraint_orientation='GLOBAL', mirror=False, proportional='DISABLED', proportional_edit_falloff='SMOOTH', proportional_size=1, gpencil_strokes=True)
        return {'FINISHED'}    

class View3D_TP_GRP_Align_Z(bpy.types.Operator):
    """Align Stroke to Z Axis"""
    bl_idname = "tp_ops.grp_stroke_align_z"
    bl_label = "Z Align"
    bl_options = {'REGISTER', 'UNDO'}
            
    def execute(self, context): 
        bpy.ops.transform.resize(value=(1, 1, 0), constraint_axis=(False, False, True), constraint_orientation='GLOBAL', mirror=False, proportional='DISABLED', proportional_edit_falloff='SMOOTH', proportional_size=1, gpencil_strokes=True)
        return {'FINISHED'} 
