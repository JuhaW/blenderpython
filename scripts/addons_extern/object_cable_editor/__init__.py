            ...
            if not wire_material_slots:
                ...
                with layout.row(True):
                    for i, material_slot in enumerate(wire_material_slots):
                        if i >= cable_settings.wire_count:
                            break
...
class CableSettingsPG:
    ...
    def propagate_layers_to_children(self):
        ...
        for sv3d in SmartView3D.find_in_ui(bpy.context.window):
            ...
            if not lv3d:
                continue
            if not BlUtil.Object.layers_intersect(v3d, obj, "layers_local_view"):
                continue
            v3ds.append(v3d)
    ...
    def _get_modifier(self, obj, md_type, create=False, name=None):
        for modifier in obj.modifiers:
            if modifier.type == md_type:
                if (not name) or (modifier.name == name):
                    return modifier
        if not create:
            return None
    ...
    def _get_constraint(self, obj, cn_type, create=False, name=None):
        for constraint in obj.constraints:
            if constraint.type == cn_type:
                if (not name) or (constraint.name == name):
                    return constraint
        if not create:
            return None
    ...
    def _get_degenerate_mesh(self, non_empty=True, name=None):
        if not name:
            name = ("DEGENERATE_MESH_NON_EMPTY" if non_empty else "DEGENERATE_MESH_EMPTY")
    ...
    def _get_child(self, obj, tag, create=False, data=None, visible=True, constraint=None):
        for child in obj.children:
            if child.cable_settings.tag == tag:
                return child
        if not create:
            return None
    ...
    def _get_driver_fcurve(self, obj, data_path, index=-1, create=False):
        ...
        if not isinstance(create, bool):
            create(fcurve)  # expected callback function
        return fcurve
    ...
    def _init_driver_single_prop(self, fcurve, id_obj=None, data_path="", driver_type=None, expression="", var_name="var"):
        if not driver_type:
            driver_type = ('SCRIPTED' if expression else 'AVERAGE')
    ...
    def _length_calculator_get(self, create=False):
        ...
        if create:
            create = self._length_calculator_init
    ...
    def _get(self):
        ...
        if curve.fill_mode != fill_mode:
            return False
        if curve.offset != 0.0:
            return False
        if curve.use_stretch:
            return False
        if curve.use_deform_bounds:
            return False
        return True
    ...
    def wire_update(self):
        ...
        if wire_type == 'BUS':
            ...
            for i in range(2):
                ...
                if not self.wire_twisting_align:
                    twist_direction_angle = 0.0
    ...
    def _set(self, value):
        ...
        wire_cap0_obj = self._get_child(wire_obj, "WIRES_CAP0")
        if wire_cap0_obj:
            wire_cap0_obj.hide_render = value
        wire_cap1_obj = self._get_child(wire_obj, "WIRES_CAP1")
        if wire_cap1_obj:
            wire_cap1_obj.hide_render = value
    ...
    def _set(self, value):
        ...
        wire_cap0_obj = self._get_child(wire_obj, "WIRES_CAP0")
        if wire_cap0_obj:
            wire_cap0_obj.hide_select = value
        wire_cap1_obj = self._get_child(wire_obj, "WIRES_CAP1")
        if wire_cap1_obj:
            wire_cap1_obj.hide_select = value
    ...
    def attachment_delete(self, index):
        ...
        for i, child in enumerate(children):
            if delete_all or (i == index):
                self._cable_child_delete(child)
    ...
    def attachment_update(self):
        ...
        if not encapsulator:
            return
        if encapsulator.cable_settings.tag != "CABLE_EXTRAS":
            return
        ...
        def drive_modifier_by_length(md, data_path, index, coefs, switch=True, inverse=False):
            ...
            if switch and (index != -1):
                switch_axis_driver(data_obj, data_path, driver_axis, size=size)
...
def cable_unselect_children(self, context, event):
    ...
    if not cable_settings.get_obj_curve():
        return
...
def scene_update_post(scene):
    ...
    if not obj:
        return
    cable_settings = obj.cable_settings
    if not cable_settings.get_spline():
        return
