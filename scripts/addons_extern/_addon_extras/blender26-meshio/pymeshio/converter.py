...
def obj_to_pmx(obj_model, name, scale):
    ...
    def create_material(i, m):
        return pmx.Material(m.name.decode("ascii"), m.name.decode("ascii"), m.Kd or common.RGB(0.5, 0.5, 1), 1.0, 1, common.RGB(1, 1, 1), common.RGB(0, 0, 0), 0, common.RGBA(0, 0, 0, 1), 0, -1, -1, pmx.MATERIALSPHERE_NONE, 1, 0, u"comment", get_vertex_count(m.faces)
                            )
