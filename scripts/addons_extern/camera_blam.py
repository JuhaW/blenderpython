...
bl_info = { \
    'name': 'BLAM - The Blender camera calibration toolkit (with fixes)',
...
class ProjectBackgroundImageOntoMeshOperator(bpy.types.Operator):
    ...
    def performSimpleProjection(self, camera, mesh, img):
        ...
        if bpy.context.scene.render.engine == 'CYCLES':
            ...
            node_emi = nodes.new('ShaderNodeEmission')
            node_tex = nodes.new('ShaderNodeTexImage')
            node_emi.location = -200,0
            node_tex.location = -400,0
            node_tex.image = img
            mat.node_tree.links.new(node_tex.outputs[0], node_emi.inputs[0])
            mat.node_tree.links.new(node_emi.outputs[0], node_out.inputs[0])
