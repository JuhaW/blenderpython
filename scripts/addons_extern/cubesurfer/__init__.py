...
def add_IsoNodeGroup(context):

    Isogroup = bpy.data.node_groups.new('IsoSurf', 'ShaderNodeTree')
    
    IsogroupOut = Isogroup.nodes.new('NodeGroupOutput')
    IsogroupOut.location = (400,200)
    Isogroup.outputs.new('NodeSocketVector','UVs')
    Isogroup.outputs.new('NodeSocketFloat','Depth')
    Isogroup.outputs.new('NodeSocketFloat','Speed')
    ...
    #bpy.data.node_groups["Shader Nodetree"].nodes["Attribute.001"].attribute_name = "IsoSurf_prop2"
    #bpy.ops.node.add_node(type="ShaderNodeCombineXYZ", use_transform=True)
    Isonode_prop1 = Isogroup.nodes.new('ShaderNodeAttribute')
    Isonode_prop1.attribute_name = 'IsoSurf_prop1'
    Isonode_prop1.location = (-200,300)
    Isonode_prop2 = Isogroup.nodes.new('ShaderNodeAttribute')
    Isonode_prop2.attribute_name = 'IsoSurf_prop2'
    Isonode_prop2.location = (-200,0)
    
    Isonode_sXYZ1 = Isogroup.nodes.new('ShaderNodeSeparateXYZ')
    Isonode_sXYZ1.location = (0,255)
    Isonode_sXYZ2 = Isogroup.nodes.new('ShaderNodeSeparateXYZ')
    Isonode_sXYZ2.location = (0,75)
    
    Isonode_cXYZ1 = Isogroup.nodes.new('ShaderNodeCombineXYZ')
    Isonode_cXYZ1.location = (200,300)
    
    Isogroup.links.new(Isonode_sXYZ1.inputs[0], Isonode_prop1.outputs[0])
    Isogroup.links.new(Isonode_sXYZ2.inputs[0], Isonode_prop2.outputs[0])
    
    Isogroup.links.new(Isonode_cXYZ1.inputs[0], Isonode_sXYZ1.outputs[0])
    Isogroup.links.new(Isonode_cXYZ1.inputs[1], Isonode_sXYZ1.outputs[1])
    Isogroup.links.new(Isonode_cXYZ1.inputs[2], Isonode_sXYZ2.outputs[0])
    
    Isogroup.links.new(Isonode_cXYZ1.outputs[0], IsogroupOut.inputs['UVs'])
    Isogroup.links.new(Isonode_sXYZ2.outputs[0], IsogroupOut.inputs['Depth'])
    Isogroup.links.new(Isonode_sXYZ2.outputs[1], IsogroupOut.inputs['Speed'])
...
def register():
    #bpy.utils.register_class(add_NodeGroup)
...
def unregister():
    #bpy.utils.unregister_class(add_NodeGroup)
...
if isosurf not in bpy.app.handlers.frame_change_post:
    ...
    bpy.app.handlers.load_post.append(add_IsoNodeGroup)
    bpy.app.handlers.persistent(add_IsoNodeGroup)
