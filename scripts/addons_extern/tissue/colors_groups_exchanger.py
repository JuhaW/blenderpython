        ...
        if(id_red <= id and id_green <= id and id_blue <= id and id_value <= id and boolCol):
            ...
            for f in obj.data.polygons:
                for v in f.vertices:
                    ...
                    if(self.red):
                        gr[min(len(gr) - sub_red, id_red)].weight = self.invert + mult * v_colors[i].color.r
                    if(self.green):
                        gr[min(len(gr) - sub_green, id_green)].weight = self.invert + mult * v_colors[i].color.g
                    if(self.blue):
                        gr[min(len(gr) - sub_blue, id_blue)].weight = self.invert + mult * v_colors[i].color.b
                    if(self.value):
                        gr[min(len(gr) - sub_value, id_value)].weight = self.invert + mult * v_colors[i].color.v
