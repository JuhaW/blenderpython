    ...
    if len(connecting_edges) == 0:
        ...
        for edge in connecting_edges:
            ...
            for f in me.polygons:
                ...
                if cvert[0] in vf and cvert[1] in vf:
                    fe += 1
            if fe > 1:
                continue
...
def qquads(f_super):
    ...
    for ix, x in enumerate(lx):
        if x < loop_q[loop_A][0].x:
            lx[ix] = loop_q[loop_A][0].x
        if x > loop_q[loop_A][3].x:
            lx[ix] = loop_q[loop_A][3].x

    for iy, y in enumerate(ly):
        if y < loop_q[loop_A][0].y:
            ly[iy] = loop_q[loop_A][0].y
        if y > loop_q[loop_A][2].y:
            ly[iy] = loop_q[loop_A][2].y
    ...
    for ix, x in enumerate(lx[:-1]):
        if abs(x - lx[ix + 1]) > maloe:
            lx_.append(x)
    for iy, y in enumerate(ly[:-1]):
        if abs(y - ly[iy + 1]) > maloe:
            ly_.append(y)
    lx_.append(lx[-1])
    ly_.append(ly[-1])
