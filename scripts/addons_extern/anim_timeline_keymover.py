...
event_types = {
    'NUMPAD_3': 3, 'ONE': 1, 'NINE': 9, 'ZERO': 0, 'THREE': 3, 'NUMPAD_2': 2,
    'NUMPAD_5': 5, 'NUMPAD_8': 8, 'NUMPAD_9': 9, 'NUMPAD_4': 4, 'NUMPAD_6': 6,
    'NUMPAD_1': 1, 'NUMPAD_0': 0, 'TWO': 2, 'FOUR': 4, 'SIX': 6, 'EIGHT': 8,
    'FIVE': 5, 'SEVEN': 7, 'NUMPAD_7': 7
}
        ...
        if context.object is None:
            ...
            if context.space_data.type == 'TIMELINE':
                ...
                for object in context.selected_objects:
                    ...
                    if object.type == 'ARMATURE' and scene.tl_only_selected_bones:
                        collect_from_bones(fcurves)
                    else:
                        collect(fcurves)

                    if object.data.animation_data is not None and object.data.animation_data.action is not None:
                        data_fcurves = object.data.animation_data.action.fcurves
...
def register():
    ...
    bpy.types.Scene.tl_mouse_sens = bpy.props.FloatProperty(
        name="Sensitivity", description="Sensitivity of cursor position to mouse movement", default=0.1, min=0.0, max=1.0
    )
    bpy.types.Scene.tl_frame_follow = bpy.props.BoolProperty(
        name="Follow Movement", description="Current frame follows movement of selected keyframe", default=True
    )
    bpy.types.Scene.tl_frame_current = bpy.props.BoolProperty(
    ...
    bpy.types.Scene.tl_only_selected_bones = bpy.props.BoolProperty(
        name="Only Selected Bones", description="Only move keys on selected bones", default=False
    )
