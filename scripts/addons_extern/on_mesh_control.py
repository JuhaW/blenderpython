...
# added group (with character rig) to the scene, to be
# at the origin of coordinates.

bl_info = {
    "name": "On Mesh Control",
    "description": "the establishment of controls attached to the mesh, "
                   "but at the same time deform the mesh.",
...
def append_to_group(context, group_name):
    ...
    for key in OnMeshControls.keys():
        ...
        if not cns_to_mesh_name in bpy.data.objects:
...
def make_control(context, name, size, radius, vtx_index=False, armature=False):
    ...
    if not red_name in armature.pose.bone_groups:
...
def register():
    for each in classes:
        bpy.utils.register_class(each)


def unregister():
    for each in classes:
        bpy.utils.unregister_class(each)
