...
class TransformAggregator:
    ...
    def OBJECT_finish(self):
        cls = self.__class__
        if self.aggr_rotation_mode.mode:
            cls.last_rotation_mode = self.aggr_rotation_mode.mode
    ...
    def EDIT_ARMATURE_process_selected(self, context, obj):
        ...
        if obj.select_head:
            self.aggr_pivots.add(m * obj.head)
        if obj.select_tail:
            self.aggr_pivots.add(m * obj.tail)
        if obj.select:
            m3 = m.to_3x3()
    ...
    def EDIT_MESH_normal_summary(self, summary):
        ...
        if self.aggr_normal_effective_count == 2:
            ...
            if normal.magnitude < 0.5:
                normal = Vector((0, 0, 1))  # just some fallback
    ...
    def EDIT_CURVE_process_selected(self, context, obj):
        ...
        if xyztw[0]:
            self.aggr_normal_x.add(xyztw[0])
        if xyztw[1]:
            self.aggr_normal_y.add(xyztw[1])
        if xyztw[2]:
            self.aggr_normal_z.add(xyztw[2])
...
def LockPG(default_uniformity=False):
    ...
    if disable_uniformity:
        default_uniformity = True
...
def SummaryVectorPG(title, axes, is_rotation=False, folded=False, resetable=True, extra_operators=None):
    ...
    class cls:
        ...
        def _set_lock_uniformity(self, value):
            for i, axis_name in enumerate(self.axis_names):
                if self.axis_defaults_uniformity[i] is None:
                    continue  # this axis should NEVER participate in uniformity
...
@addon.Operator(idname="object.batch_transform_vector_menu", options={'INTERNAL'}, description="Click: Vector menu")
...
@addon.Operator(idname="object.batch_transform_reset_vectors", options={'REGISTER', 'UNDO'}, description="Click: reset all axes (+Ctrl: respect locks, +Shift: reset all vectors)", space_type='VIEW_3D')
def Operator_Reset_Vectors(self, context, event, vectors=""):
    ...
    tfm_mode = transform.get_transform_mode(context.mode)
    if tfm_mode:
        tfm_mode.reset_vectors(vectors, not UIMonitor.ctrl)
...
class BaseTransformPG:
    ...
    def begin(self, transform, vector_name, axis_name, summary_index, uniformity):
        ...
        if tfm_aggr is None:
            return  # no aggr yet, can't do anything
    ...
    def reset_vectors(self, vector_names=None, ignore_locks=True):
        ...
        for vector_name in self.vector_names:
            if (vector_names is not None) and (vector_name not in vector_names):
                continue

            vector_prop = getattr(self, vector_name)
            if not vector_prop.resetable:
                continue
...
@addon.Operator(idname="object.batch_transform_summary", options={'INTERNAL'}, description="Click: Summary menu")
...
@addon.Operator(idname="object.batch_transform_summary_copy", options={'INTERNAL'}, description="Click: Copy")
def Operator_Summary_Copy(self, context, event):
    category = get_category()
    options = get_options()


@addon.Operator(idname="object.batch_transform_summary_paste", options={'INTERNAL'}, description="Click: Paste")
def Operator_Summary_Paste(self, context, event):
    category = get_category()
    options = get_options()


@addon.Operator(idname="object.batch_transform_property", options={'INTERNAL'}, description="Click: Property menu")
...
class ContextTransformPG:
    ...
    def apply(self, tfm_aggr):
        ...
        if tfm_mode:
            tfm_mode.apply(tfm_aggr, summaries)
    ...
    def draw(self, layout):
        ...
        with layout.row(True):
            for item in self.summary_items:
                if item[0] in self.summaries:
                    text = item[1]
                    if item[0] == 'active':
                        if self.uniformity == 'OFFSET':
                            text = "+" + text
                        elif self.uniformity == 'PROPORTIONAL':
                            text = "* " + text
            ...
            if not self.summaries:
                layout.label(" ")  # just to fill space
        ...
        if tfm_mode:
            tfm_mode.draw(layout, self.summaries)
    ...
    def find_transform(self, screen, area):
        ...
        while searches > 0:
            for i in range(len(areas)):
                ...
                found = True
            if not found:
                self.transforms_ensure_order(screen)
            searches -= 1
...
@addon.Operator(idname="view3d.pick_transforms", options={'INTERNAL', 'REGISTER'}, description="Pick transform(s) from the object under mouse")
...
@addon.Operator(idname="object.batch_transform_copy", options={'INTERNAL'}, description="Click: Copy")
...
@addon.Operator(idname="object.batch_transform_paste", options={'INTERNAL', 'REGISTER'}, description="Click: Paste (+Ctrl: Override, +Shift: Add, +Alt: Filter)")
