...
class CoordSystemMatrix:
    ...
    def update(self, coordsys, sv=None, context=None):
        ...
        if (sv is None) and context:
            ...
            if ui_context:
                sv = SmartView3D(use_camera_axes=True, **ui_context)
    ...
    def _gimbal_xyz(self, context, am):
        ...
        if (not rotobj) or (rotobj.rotation_mode == 'QUATERNION'):
            if not am:
                am = self.calc_active_matrix(context)
            xyz = am.col[:3]
    ...
    def calc_matrix(self, context, obj, local_xyzt=None):
        ...
        if not t:
            if L_mode == 'LOCAL':
                if self.active_xyzt:  # in mesh/curve/lattice/bone edit modes, treat active object as parent
                    ...
                    if not pm:
                        pm = BlUtil.Object.matrix_parent(obj)
                    t = pm.translation
            elif L_mode == 'PARENT':
                if not pm:
                    pm = BlUtil.Object.matrix_parent(obj)
                ...
                if not lm:
                    lm = BlUtil.Object.matrix_world(obj)
                ...
                if not wplm:
                    wplm = self.workplane_matrix(context, True)
                t = wplm.translation
                self._cached_t = t
            elif L_mode == 'CURSOR':
        ...
        if R_mode == 'ORIENTATION':
            ...
            if not v3d.current_orientation:
                R_mode = ormap.get(v3d.transform_orientation, v3d.transform_orientation)
    ...
    def set_LRS(self, context, obj, LRS, rotation_mode='QUATERNION'):
        ...
        if R:
            BlUtil.Object.rotation_apply(obj, R, rotation_mode)
        if S:
            obj.scale = Vector(S)
...
class CoordSystemPG:
    ...
    def btn_pick_aspect(context, event, arg):
        ...
        for aspect in aspects:
            if aspect.mode not in {'OBJECT', 'CAMERA'}:
                aspect.mode = 'OBJECT'
...
@addon.Operator(idname="view3d.coordsystem_new", options={'INTERNAL', 'REGISTER'}, description="Click: copy current coordsystem, Ctrl+Click: bake current coordsystem, Shift+Click: bake manipulator, Alt+Click: bake workplane")
...
class CoordSystemManagerPG:
    ...
    def draw_view():
        context = bpy.context
        manager = get_coordsystem_manager(context)
        if not (manager.show_grid_xy or manager.show_grid_yz or manager.show_grid_xz):
            return
        ...
        with cgl(LineWidth=1, DepthMask=False, DEPTH_TEST=False, BLEND=True, LINE_STIPPLE=False):
            with cgl.batch('LINES') as batch:
                ...
                if manager.show_grid_xy:
                    drawgrid(batch, 0, 1, color_xy)
                if manager.show_grid_yz:
                    drawgrid(batch, 1, 2, color_yz)
                if manager.show_grid_xz:
                    drawgrid(batch, 0, 2, color_xz)
    del draw_view
