...
class drawPoly(bpy.types.Operator):
    """Draw a polygon / LMB = Draw / +CTRL = Snap / SHIFT = Flip Normals / MMB = Fill Beauty / RMB = ESC"""
...
class paintDeSelect(bpy.types.Operator):
    """Pain Deselect = Hold LMB+CTRL+ALT"""
    bl_idname = "view3d.deselect_paint"
    bl_label = "Paint DeSelect"
    bl_options = {'REGISTER', 'UNDO'} 

    deselect = BoolProperty(default = True, description = 'Deselect objects, polys, edges or verts')
    toggle = BoolProperty(default = False, description = 'Toggles the selection. NOTE: this option can be slow on heavy meshes')
    sel_before = IntProperty(description = 'Do Not Touch', options = {'HIDDEN'})
    sel_after = IntProperty(description = 'Do Not Touch', options = {'HIDDEN'})
...
class pathCollpaseSelectRing(bpy.types.Operator):
    """Collapses everything between your two selected edges / only edge mode"""
    bl_idname = "mesh.path_collapse_select_ring"
...
def register():
        ...
        bpy.utils.register_class(paintDeSelect)
        bpy.utils.register_class(pathSelectRing)
        bpy.utils.register_class(pathCollpaseSelectRing)
        ...
        if kc:
            ...
            """
            # Add paint select to ALT+LeftMouse
            km = kc.keymaps.new(name="3D View", space_type="VIEW_3D")
            kmi = km.keymap_items.new('view3d.select_paint', 'ACTIONMOUSE', 'PRESS', alt=True)
            # Add paint deselect to CTRL+ALT+LeftMouse
            km = kc.keymaps.new(name="3D View", space_type="VIEW_3D")
            kmi = km.keymap_items.new('view3d.deselect_paint', 'ACTIONMOUSE', 'PRESS', ctrl=True, alt=True)            
            """
...
def unregister():
        ...
        bpy.utils.unregister_class(paintDeSelect)
        bpy.utils.unregister_class(pathSelectRing)
        bpy.utils.unregister_class(pathCollpaseSelectRing)
        ...
        if kc:
            ...
            km = kc.keymaps["3D View"]
            for kmi in km.keymap_items:
                if kmi.idname == 'view3d.deselect_paint':
                    km.keymap_items.remove(kmi)
                    break
            """
