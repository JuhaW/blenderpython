...
def draw_wkst_object_ui(self, context, layout):
...
#Specials
     ...
     if obj:
        ...
        if obj_type in {'MESH'}:  
            ...
            row.operator("objects.multiedit_enter_operator", icon="BLANK1")
        ...
        if obj_type in {'ARMATURE', 'EMPTY', 'CAMERA', 'LAMP', 'SPEAKER'}:
            box = layout.box().column(1)
            row = box.row(1)    
            row.label("Custom Place...")  
