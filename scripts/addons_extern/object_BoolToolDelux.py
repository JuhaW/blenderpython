...
class BTool_Slice(bpy.types.Operator):
    """This operator add a intersect brush to a canvas"""
...
# Boolean Slice Operator
class BTool_Slice_Direct(bpy.types.Operator):
    """Make the difference between selected objects to the active"""
    bl_idname = "btool.boolean_slice_direct"
    bl_label = "Direct Slice"
    ...
    def execute(self, context):
        Operation_Direct(context, "SLICE")
        return {'FINISHED'}
...
class BoolTool_Tools(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        col.operator(BTool_Slice_Direct.bl_idname, text="Slice", icon="ROTATECENTER")
...
class BoolTool_Config(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if isBrush(actObj):
            ...
            if (actObj["BoolToolBrush"] == "UNION") or (actObj["BoolToolBrush"] == "SLICE"):
...
def register():
    ...
    bpy.utils.register_class(BTool_Slice)
    ...
    bpy.utils.register_class(BTool_Slice_Direct)
    ...
    addon_keymaps.append((km, kmi))
    kmi = km.keymap_items.new(BTool_Slice_Direct.bl_idname, 'NUMPAD_SLASH', 'PRESS', ctrl=True, shift=True)
    addon_keymaps.append((km, kmi))
