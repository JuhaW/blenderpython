...
class SocketUiList(bpy.types.UIList):
    ...
    def draw_item(self, context, layout, node, socket, icon, activeData, activePropname):
        ...
        if socket.textProps.editable:
            ...
            layout.label(socket.getDisplayedName())
...
def moveSocket(isOutput, moveUp):
    ...
    node = socket.node
    if isOutput:
        node.activeOutputIndex = list(node.outputs).index(socket)
    else:
        node.activeInputIndex = list(node.inputs).index(socket)
    return {"FINISHED"}
...
def getActiveSocket(isOutput):
    ...
    if node is None:
        return
    if isOutput:
        return node.activeOutputSocket
    else:
        return node.activeInputSocket
