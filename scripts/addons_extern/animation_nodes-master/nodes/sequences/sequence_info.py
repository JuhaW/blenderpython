...
class SequenceInfoNode(bpy.types.Node, AnimationNode):
    ...
    def getExecutionCode(self):
        ...
        if isLinked["opacity"]:
            yield "    opacity = sequence.blend_alpha"
        if isLinked["blendType"]:
            yield "    blendType = sequence.blend_type"
        if isLinked["channel"]:
            yield "    channel = sequence.channel"
        if isLinked["effectFader"]:
            yield "    effectFader = sequence.effect_fader"

        if isLinked["totalDuration"]:
            yield "    totalDuration = sequence.frame_duration"
        if isLinked["finalDuration"]:
            yield "    finalDuration = sequence.frame_final_duration"
        if isLinked["finalStartFrame"]:
            yield "    finalStartFrame = sequence.frame_final_start"
        if isLinked["finalEndFrame"]:
            yield "    finalEndFrame = sequence.frame_final_end"
        if isLinked["startOffset"]:
            yield "    startOffset = sequence.frame_offset_start"
        if isLinked["endOffset"]:
            yield "    endOffset = sequence.frame_offset_end"
        if isLinked["startFrame"]:
            yield "    startFrame = sequence.frame_start"
        if isLinked["stillFrameStart"]:
            yield "    stillFrameStart = sequence.frame_still_start"
        if isLinked["stillFrameEnd"]:
            yield "    stillFrameEnd = sequence.frame_still_end"

        if isLinked["lock"]:
            yield "    lock = sequence.lock"
        if isLinked["mute"]:
            yield "    mute = sequence.mute"
        if isLinked["name"]:
            yield "    name = sequence.name"
        if isLinked["select"]:
            yield "    select = sequence.select"
        if isLinked["speedFactor"]:
            yield "    speedFactor = sequence.speed_factor"
        if isLinked["type"]:
            yield "    type = sequence.type"
        if isLinked["useDefaultFade"]:
            yield "    useDefaultFade = sequence.use_default_fade"
        if isLinked["useLinearModifiers"]:
            yield "    useLinearModifiers = sequence.use_linear_modifiers"
