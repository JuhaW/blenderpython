...
class ObjectMatrixInputNode(bpy.types.Node, AnimationNode):
    ...
    def getExecutionCode(self):
        ...
        if isLinked["parentInverse"]:
            lines.append("    parentInverse = mathutils.Matrix.Identity(4)")
        lines.append("else:")
        if isLinked["world"]:
            lines.append("    world = object.matrix_world")
        if isLinked["basis"]:
            lines.append("    basis = object.matrix_basis")
        if isLinked["local"]:
            lines.append("    local = object.matrix_local")
        if isLinked["parentInverse"]:
            lines.append("    parentInverse = object.matrix_parent_inverse")
