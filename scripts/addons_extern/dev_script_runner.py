    ...
    def execute(self, context):
        ...
        if (self.filepath == ""):
            ...
            if os.path.isfile(self.filepath):
                ...
                if self.script_slot == 1:
                    user_settings.script0_path = self.filepath
                if self.script_slot == 2:
                    user_settings.script1_path = self.filepath
                if self.script_slot == 3:
                    user_settings.script2_path = self.filepath
                if self.script_slot == 4:
                    user_settings.script3_path = self.filepath
                if self.script_slot == 5:
                    user_settings.script4_path = self.filepath
                if self.script_slot == 6:
                    user_settings.script5_path = self.filepath
                if self.script_slot == 7:
                    user_settings.script6_path = self.filepath
                if self.script_slot == 8:
                    user_settings.script7_path = self.filepath
                if self.script_slot == 9:
                    user_settings.script8_path = self.filepath
                if self.script_slot == 10:
                    user_settings.script9_path = self.filepath
...
class ScriptRun(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if self.script_slot == 0:
            file_path = user_settings.script0_path
        if self.script_slot == 1:
            file_path = user_settings.script1_path
        if self.script_slot == 2:
            file_path = user_settings.script2_path
        if self.script_slot == 3:
            file_path = user_settings.script3_path
        if self.script_slot == 4:
            file_path = user_settings.script4_path
        if self.script_slot == 5:
            file_path = user_settings.script5_path
        if self.script_slot == 6:
            file_path = user_settings.script6_path
        if self.script_slot == 7:
            file_path = user_settings.script7_path
        if self.script_slot == 8:
            file_path = user_settings.script8_path
        if self.script_slot == 9:
            file_path = user_settings.script9_path
...
def clearSlot(script_slot):
    ...
    if script_slot == 0:
        user_settings.script0_path = ""
    if script_slot == 1:
        user_settings.script1_path = ""
    if script_slot == 2:
        user_settings.script2_path = ""
    if script_slot == 3:
        user_settings.script3_path = ""
    if script_slot == 4:
        user_settings.script4_path = ""
    if script_slot == 5:
        user_settings.script5_path = ""
    if script_slot == 6:
        user_settings.script6_path = ""
    if script_slot == 7:
        user_settings.script7_path = ""
    if script_slot == 8:
        user_settings.script8_path = ""
    if script_slot == 9:
        user_settings.script9_path = ""
...
def checkFiles():
    ...
    if not os.path.isfile(user_settings.script0_path):
        user_settings.script0_path = ""
    if not os.path.isfile(user_settings.script1_path):
        user_settings.script1_path = ""
    if not os.path.isfile(user_settings.script2_path):
        user_settings.script2_path = ""
    if not os.path.isfile(user_settings.script3_path):
        user_settings.script3_path = ""
    if not os.path.isfile(user_settings.script4_path):
        user_settings.script4_path = ""
    if not os.path.isfile(user_settings.script5_path):
        user_settings.script5_path = ""
    if not os.path.isfile(user_settings.script6_path):
        user_settings.script6_path = ""
    if not os.path.isfile(user_settings.script7_path):
        user_settings.script7_path = ""
    if not os.path.isfile(user_settings.script8_path):
        user_settings.script8_path = ""
    if not os.path.isfile(user_settings.script9_path):
        user_settings.script9_path = ""
