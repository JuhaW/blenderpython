...
# FACES ---------------------------
    ...
    for Y in range(0, my):
        ...
        for i in range(0, mx * 4 + 2, 4):
            fc.extend([[z + i + 1, z + i + 0, z + i + V + 2, z + i + V + 3], [z + i + 3, z + i + 1, z + i + V + 3, z + i + V + 5], [z + i + 2, z + i + 3, z + i + V + 5, z + i + V + 4], [z + i + 0, z + i + 2, z + i + V + 4, z + i + V + 2]])
    # Fitil-------------------
    if self.UST == '1':
        y1 = my
    else:
    ...
    if self.UST == '1':  # Duz
        ...
        for i in range(0, len(U1) - 1):
            fc.append([U1[i + 1], U1[i], U2[i], U2[i + 1]])
            SM.append(len(fc) - 1)
        for A in range(0, mx):
            for i in range(0, len(D1[A]) - 1):
                fc.append([D1[A][i + 1], D1[A][i], D2[A][i], D2[A][i + 1]])
                SM.append(len(fc) - 1)
        y = my - 1
        for x in range(0, mx):
            if kx[x][y] == True:
                ...
                vr[ALT[0]][2] = sqrt(R**2 - vr[ALT[0]][0]**2) + z
                vr[ALT[1]][2] = sqrt(K**2 - vr[ALT[1]][0]**2) + z
                vr[ALT[2]][2] = sqrt(K**2 - vr[ALT[2]][0]**2) + z
                vr[ALT[3]][2] = sqrt(R**2 - vr[ALT[3]][0]**2) + z
                vr[ALT[4]][2] = sqrt(R**2 - vr[ALT[4]][0]**2) + z
                vr[ALT[5]][2] = sqrt(K**2 - vr[ALT[5]][0]**2) + z
                vr[ALT[6]][2] = sqrt(K**2 - vr[ALT[6]][0]**2) + z
                vr[ALT[7]][2] = sqrt(R**2 - vr[ALT[7]][0]**2) + z
                ...
                for i in range(0, RES):
                    ...
                    y2 = -cos(A) * K
                    if x1 < y1 and y1 < x2:
                        vr.extend([[y1, fr - k2 / 2, sin(A) * R + z], [y1, fr + k2 / 2, sin(A) * R + z]])
                        T1.append(len(vr) - 2)
                        T2.append(len(vr) - 1)
                    if x1 + k2 < y2 and y2 < x2 - k2:
                        vr.extend([[y2, fr - k2 / 2, sin(A) * K + z], [y2, fr + k2 / 2, sin(A) * K + z]])
                        D1.append(len(vr) - 2)
                        D2.append(len(vr) - 1)
                ON = [ALT[1], ALT[0]]
                ON.extend(T1)
                ...
                if D1 == [] and T1 == []:
                    fc.extend([ON, AR, [ALT[5], ALT[6], ALT[2], ALT[1]], [ALT[7], ALT[4], ALT[0], ALT[3]]])
                    m = len(fc)
                    SM.extend([m - 1, m - 2])
                elif D1 == [] and T1 != []:
                    fc.extend([ON, AR, [ALT[5], ALT[6], ALT[2], ALT[1]], [ALT[7], ALT[4], T1[-1], T2[-1]], [ALT[0], ALT[3], T2[0], T1[0]]])
                    m = len(fc)
                    SM.extend([m - 1, m - 2, m - 3])
                elif D1 != [] and T1 == []:
                    fc.extend([ON, AR, [ALT[5], ALT[6], D2[0], D1[0]], [ALT[2], ALT[1], D1[-1], D2[-1]], [ALT[7], ALT[4], ALT[0], ALT[3]]])
                    m = len(fc)
                    SM.extend([m - 1, m - 2, m - 3])
                else:
                    fc.extend([ON, AR, [ALT[5], ALT[6], D2[0], D1[0]], [ALT[2], ALT[1], D1[-1], D2[-1]], [ALT[7], ALT[4], T1[-1], T2[-1]], [ALT[0], ALT[3], T2[0], T1[0]]])
                    m = len(fc)
                    SM.extend([m - 1, m - 2, m - 3, m - 4])

                for i in range(0, len(D1) - 1):
                    fc.append([D1[i + 1], D1[i], D2[i], D2[i + 1]])
                    SM.append(len(fc) - 1)
                for i in range(0, len(T1) - 1):
                    fc.append([T1[i + 1], T1[i], T2[i], T2[i + 1]])
                    SM.append(len(fc) - 1)
                R = C - k1 - k2
                K = R - k3 * 2
            else:
            ...
            for i in range(0, RES):
                ...
                y2 = -cos(A) * K
                if x1 < y1 and y1 < x2:
                    vr.extend([[y1, fr - k3, sin(A) * R + z], [y1, fr + k3, sin(A) * R + z]])
                    T1.append(len(vr) - 2)
                    T2.append(len(vr) - 1)
                    ftl.extend([len(fc) - 1, len(fc) - 2])
                if x1 + k3 * 2 < y2 and y2 < x2 - k3 * 2:
                    vr.extend([[y2, fr - k3, sin(A) * K + z], [y2, fr + k3, sin(A) * K + z]])
                    D1.append(len(vr) - 2)
                    D2.append(len(vr) - 1)
                    ftl.extend([len(fc) - 1, len(fc) - 2])
            ON = [ALT[1], ALT[0]]
            ...
            if D1 == []:
                ...
                fc.extend([ON, AR, [ALT[5], ALT[6], D2[0], D1[0]], [ALT[2], ALT[1], D1[-1], D2[-1]]])
                m = len(fc)
        ...
        if self.DT3 == '1':
            ...
            H = sin(self.VL4 * pi / 180) / cos(self.VL4 * pi / 180)
        z = sqrt(k1**2 + (k1 * H)**2)
        k = sqrt(k2**2 + (k2 * H)**2)
        ...
        for j in range(2, len(ALT) - 2, 4):
            if vr[ALT[j]][0] < 0 and 0 < vr[ALT[j + 2]][0]:
                x = 1
        ...
        if x == 0:
            vr.extend([[0, -k1 / 2, Z[-1] - z], [0, k1 / 2, Z[-1] - z]])
    ...
    if self.mr == True:
        ...
        n = len(vr)
        fc.extend([[n - 1, n - 2, n - 4, n - 3], [n - 3, n - 4, n - 8, n - 7], [n - 6, n - 5, n - 7, n - 8], [n - 2, n - 1, n - 5, n - 6], [n - 4, n - 2, n - 6, n - 8], [n - 5, n - 1, n - 3, n - 7]])
        n = len(fc)
...
# OBJE -----------------------------------------------------------
    ...
    if self.mt1 == '1':
        mesh.materials.append(MAT('PVC', 1.0, 1.0, 1.0))
    elif self.mt1 == '2':
        mesh.materials.append(MAT('Wood', 0.3, 0.2, 0.1))
    elif self.mt1 == '3':
        mesh.materials.append(MAT('Plastic', 0.0, 0.0, 0.0))
    ...
    if self.mr == True:
        mesh.materials.append(MAT('Marble', 0.9, 0.8, 0.7))
...
class PENCERE(bpy.types.Operator):
    ...
    k00 = BoolProperty(name='', default=True)
    k01 = BoolProperty(name='', default=False)
    k02 = BoolProperty(name='', default=True)
    k03 = BoolProperty(name='', default=False)
    k04 = BoolProperty(name='', default=False)
    k05 = BoolProperty(name='', default=False)
    k06 = BoolProperty(name='', default=False)
    k07 = BoolProperty(name='', default=False)

    k10 = BoolProperty(name='', default=False)
    k11 = BoolProperty(name='', default=False)
    k12 = BoolProperty(name='', default=False)
    k13 = BoolProperty(name='', default=False)
    k14 = BoolProperty(name='', default=False)
    k15 = BoolProperty(name='', default=False)
    k16 = BoolProperty(name='', default=False)
    k17 = BoolProperty(name='', default=False)

    k20 = BoolProperty(name='', default=False)
    k21 = BoolProperty(name='', default=False)
    k22 = BoolProperty(name='', default=False)
    k23 = BoolProperty(name='', default=False)
    k24 = BoolProperty(name='', default=False)
    k25 = BoolProperty(name='', default=False)
    k26 = BoolProperty(name='', default=False)
    k27 = BoolProperty(name='', default=False)

    k30 = BoolProperty(name='', default=False)
    k31 = BoolProperty(name='', default=False)
    k32 = BoolProperty(name='', default=False)
    k33 = BoolProperty(name='', default=False)
    k34 = BoolProperty(name='', default=False)
    k35 = BoolProperty(name='', default=False)
    k36 = BoolProperty(name='', default=False)
    k37 = BoolProperty(name='', default=False)

    k40 = BoolProperty(name='', default=False)
    k41 = BoolProperty(name='', default=False)
    k42 = BoolProperty(name='', default=False)
    k43 = BoolProperty(name='', default=False)
    k44 = BoolProperty(name='', default=False)
    k45 = BoolProperty(name='', default=False)
    k46 = BoolProperty(name='', default=False)
    k47 = BoolProperty(name='', default=False)
    ...
    def draw(self, context):
        ...
        row = layout.row()
        box.row()
        row.prop(self, 'rotationPropX')
        row.prop(self, 'rotationPropY')
        row.prop(self, 'rotationPropZ')
        row = layout.row()
        row.label("Location")
        row = layout.row()
        box.row()
        row.prop(self, 'locationPropX')
        row.prop(self, 'locationPropY')
        row.prop(self, 'locationPropZ')
