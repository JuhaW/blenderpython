...
class Erode(bpy.types.Operator):
    ...
    def draw(self, context):
        ...
        if self.showiterstats:
            ...
            col2 = row.column()
            col1.label("Time")
            col2.label("%.1f s" % self.stats.elapsedtime)
            if self.stats.memstats_available:
                col1.label("Memory")
                col2.label("%.1f Mb" % (self.stats.maxmem / (1024.0 * 1024.0)))
            col1.label("Diffusions")
            col2.label("%d" % self.counts['diffuse'])
            col1.label("Avalanches")
            col2.label("%d" % self.counts['avalanche'])
            col1.label("Water movements")
            col2.label("%d" % self.counts['water'])
        box = layout.box()
