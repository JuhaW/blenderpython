...
def tile_hexagon(ow, ol, tw, s, th):
    ...
    while cur_y < ol + tw:
        ...
        while cur_x < ow:
            ...
            n = p
            for i in range(5):
                faces.append((n, n + 1, n + 7, n + 6))
                n += 1
            faces.append((p + 5, p, p + 6, p + 11))
            f = ((p + 3, p + 2, p + 1, p), (p + 5, p + 4, p + 3, p), (p + 6, p + 7, p + 8, p + 9), (p + 9, p + 10, p + 11, p + 6))
            for face in f:
                faces.append(face)
            cur_x += tw + s
        cur_y += r + h + s
            ...
            if orient == "length":
                ...
                for i in range(num_boards):
                    ...
                    if cur_x < ow and cur_y < ol:
                        ...
                        if cur_x < ow and cur_x + bw > ow:
                            bw2 = ow - cur_x
                        if cur_y < ol and cur_y + bl > ol:
                            bl2 = ol - cur_y
                        p = len(verts)
                        v = ((cur_x, cur_y, 0.0), (cur_x, cur_y, z), (cur_x + bw2, cur_y, z), (cur_x + bw2, cur_y, 0.0))
                        cur_y += bl2
                        v += ((cur_x, cur_y, 0.0), (cur_x, cur_y, z), (cur_x + bw2, cur_y, z), (cur_x + bw2, cur_y, 0.0))
                        cur_y -= bl2
                        cur_x += bw2 + s
                ...
                for i in range(num_boards):
                    ...
                    bw2 = bw
                    if cur_x < ow and cur_y < ol:
                        if cur_x < ow and cur_x + bl > ow:
                            bl2 = ow - cur_x
                        if cur_y < ol and cur_y + bw > ol:
                            bw2 = ol - cur_y
                        p = len(verts)
                        v = ((cur_x, cur_y + bw2, 0.0), (cur_x, cur_y + bw2, z), (cur_x, cur_y, z), (cur_x, cur_y, 0.0))
                        cur_x += bl2
                        v += ((cur_x, cur_y + bw2, 0.0), (cur_x, cur_y + bw2, z), (cur_x, cur_y, z), (cur_x, cur_y, 0.0))
                        cur_x -= bl2
                        cur_y += bw2 + s
...
def FlooringMaterial(self, context):
    ...
    if o.f_col_image == "":
        ...
        error = True
    if o.f_is_bump == True and o.f_norm_image == "":
