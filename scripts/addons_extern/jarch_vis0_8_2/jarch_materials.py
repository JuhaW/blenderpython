...
def GlossyDiffuse(bpy, dif_col, glos_col, rough, mix, name):
    ...
    rgba = list(dif_col)
    rgba.append(1.0)
    mat.diffuse_color = dif_col  # create rbga variable and set diffuse color
...
def Image(bpy, context, im_scale, c_image, n_image, bump_amo, is_bump, name, is_gloss, glossy, mix, rotate, extra_rot):
    ...
    for i in bpy.data.images:
        if i.filepath == c_image and enter == False:
            col_im = i
            enter = True  # check if color image already loaded
        elif i.filepath == n_image and enter2 == False and is_bump == True:
            norm_im = i
            enter2 = True  # check if normal image already loaded
