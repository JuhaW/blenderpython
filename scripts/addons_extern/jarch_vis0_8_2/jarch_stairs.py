...
def create_stairs(self, context, style, overhang, steps, tw, rh, of, os, w, n_landing, is_close, tw0, rh0,
    ...
    if style == "1":
        ...
        pre_pos = [0.0, 0.0, 0.0]
        pre_rot = context.object.rotation_euler.copy()
            ...
            if i == 0:
                ...
                if pass_in[6] == "1":  # forwards
                    ...
                    # level 2
                        ...
                        pre_pos[1] += pass_in[7]
                    elif i == 2 and l_rot0 == "2" and is_back0 == False:
                        pre_pos[0] -= pass_in[7]
                    elif i == 2 and l_rot0 == "2" and is_back0 == True:
                        pre_pos[1] -= pass_in[7]
                    elif i == 2 and l_rot0 == "3" and is_back0 == False:
                        pre_pos[0] += pass_in[7]
                    elif i == 2 and l_rot0 == "3" and is_back0 == True:
                        pre_pos[1] -= pass_in[7]
                elif pass_in[6] == "2":  # left
                    # level 1
                    if i == 1 and is_back0 == False:
                        pre_pos[0] -= w / 2
                        pre_pos[1] += pass_in[7] / 2
                        type = "straight"
                    elif i == 1 and is_back0 == True:
                        pre_pos[0] -= w
                        type = "left"
                    # level 2
                    # first goes forwards
                    elif i == 2 and l_rot0 == "1" and is_back1 == False:
                        pre_pos[1] += pass_in[7] / 2
                        pre_pos[0] -= w / 2
                        type = "straight"
                    elif i == 2 and l_rot0 == "1" and is_back1 == True:
                        pre_pos[0] -= w
                        type = "left"
                    # first goes left
                    elif i == 2 and l_rot0 == "2" and is_back0 == False and is_back1 == False:
                        pre_pos[0] -= pass_in[7] / 2
                        pre_pos[1] -= w / 2
                        type = "straight"
                    elif i == 2 and l_rot0 == "2" and is_back0 == False and is_back1 == True:
                        pre_pos[1] -= w
                        type = "left"
                    elif i == 2 and l_rot0 == "2" and is_back0 == True and is_back1 == False:
                        pre_pos[0] += w / 2
                        pre_pos[1] -= pass_in[7] / 2
                        type = "straight"
                    elif i == 2 and l_rot0 == "2" and is_back0 == True and is_back1 == True:
                        pre_pos[0] += w
                        type = "left"
                    # first goes right
                    elif i == 2 and l_rot0 == "3" and is_back0 == False and is_back1 == False:
                        pre_pos[0] += pass_in[7] / 2
                        pre_pos[1] += w / 2
                        type = "straight"
                    elif i == 2 and l_rot0 == "3" and is_back0 == False and is_back1 == True:
                        pre_pos[1] += w
                        type = "left"
                    elif i == 2 and l_rot0 == "3" and is_back0 == True and is_back1 == False:
                        pre_pos[0] += w / 2
                        pre_pos[1] -= pass_in[7] / 2
                        type = "straight"
                    elif i == 2 and l_rot0 == "3" and is_back0 == True and is_back1 == True:
                        pre_pos[0] += w
                        type = "left"
                ...
                if is_landing == True:
                    ...
                    vpos2 = Vector(pre_pos2) * matrix
                    pos2 = list(vpos2)
                    pos2[0] += o.location[0]
                    pos2[1] += o.location[1]
                    pos2[2] += o.location[2]
                    ob2.location = pos2
                    ob2.rotation_euler = pre_rot
                    ob2.scale = o.scale
                    pre_rot = eur
...
def spiral_stairs(self, context, w, steps, tw, rh, rot, of, tread_res, pd, pole_res):
    ...
    for step in range(steps - 1):
        ...
        for i in points:
            ...
            if counter == 1 and rot >= 0:
                angle = cur_ang - e_rot
            elif counter == 2 + tread_res and rot >= 0:
                angle = cur_ang + ang
            # if negative rotation
            elif counter == 1 and rot < 0:
                angle = cur_ang + ang
            elif counter == 2 + tread_res and rot < 0:
                angle = cur_ang
            # middle vertices
            elif counter > 1 and counter < 2 + tread_res and rot >= 0:
                angle = cur_ang + (((ang + e_rot) / (tread_res + 1)) * (counter - 1)) - e_rot
            elif counter > 1 and counter < 2 + tread_res and rot < 0:
                angle = (cur_ang + ang) - (((ang + e_rot) / (tread_res + 1)) * (counter - 1)) - e_rot
            else:
                angle = step * ang
                angle += radians(180)
                angle *= -1
        ...
        for i in range(4 + (tread_res * 2)):
            if i != 3 + (tread_res * 2):
                f.append((lp, lp + 2, lp + 3, lp + 1))  # if not last face
            else:
                f.append((lp, p, p + 1, lp + 1))  # if last face
            lp += 2  # update local p
        # calculate top faces
        op = []
        ip = [p + 1]
        for i in range(p + 3, p + 3 + (tread_res * 2) + 4, 2):
            op.append(i)
        for i in range(p + 7 + (tread_res * 4), p + 7 + (tread_res * 4) - (tread_res * 2) - 2, -2):
            ip.append(i)
        for i in range(tread_res + 1):
            f.append((ip[i], op[i], op[i + 1], ip[i + 1]))
        # bottom faces
        op = []
        ip = [p]
        for i in range(p + 2, p + 2 + (tread_res * 2) + 4, 2):
            op.append(i)
        for i in range(p + 6 + (tread_res * 4), p + 6 + (tread_res * 4) - (tread_res * 2) - 2, -2):
            ip.append(i)
        for i in range(tread_res + 1):
            f.append((ip[i], ip[i + 1], op[i + 1], op[i]))
        for face in f:
            faces.append(face)
    # pole
    cz -= rh - t
...
def winding_stairs(self, context, tw, rh, of, w, steps, is_close, w_rot, stair_to_rot):
    ...
    w_rot = round(w_rot, 4)
    r45 = round(radians(45), 4)
    r180 = round(radians(180), 4)
    r90 = round(radians(90), 4)
    # winding stairs
    verts = []
    faces = []
    inch = 1 / 39.3701
    ...
    hw = w / 2
    rh -= t
    ay = (steps - str - c_steps) * tw
    az = (steps - str) * rh  # ay is the distance from the corner to the top of the stairs
    temp_dw = sqrt((gy ** 2 + w ** 2))
    temp_x = 0.0
    if -r45 <= w_rot <= r45:
        dw = temp_dw
    else:
    ...
    for step in range(steps):
        ...
        if step + 1 < str:  # before rotation
            ...
            if -r45 <= w_rot <= r45:
                ...
                if 0 < w_rot <= r45:  # positive rotation
                    cx = hw
                    v = ((cx, cy, cz), (cx, cy, cz + rh), (cx - w, cy + y2, cz), (cx - w, cy + y2, cz + rh), (cx - w, cy + y2 + t, cz), (cx - w, cy + y2 + t, cz + rh), (cx, cy + t, cz), (cx, cy + t, cz + rh))
                    cz += rh
                    v += ((cx, cy - of, cz), (cx, cy - of, cz + t), (cx - w, cy + y2 - of, cz), (cx - w, cy + y2 - of, cz + t), (cx - w, cy + y, cz), (cx - w, cy + y, cz + t), (cx, cy + t, cz), (cx, cy + t, cz + t))
                    face_type = "pos"
                ...
                ang = w_rot / (c_steps + 1)
                cs = step + 1 - str
                cur_ang = ang * (step + 2 - str)
                if w_rot > 0:  # positive rotation
                    if abs(cur_ang) <= r45:
                        y = (gy / r45) * abs(cur_ang)
                        y2 = (gy / r45) * (abs(cur_ang - ang))
                        cx = hw
                        v = ((cx, cy, cz), (cx, cy, cz + rh), (cx - w, cy + y2, cz), (cx - w, cy + y2, cz + rh), (cx - w, cy + y2 + t, cz), (cx - w, cy + y2 + t, cz + rh), (cx, cy + t, cz), (cx, cy + t, cz + rh))
                        cz += rh
                        v += ((cx, cy - of, cz), (cx, cy - of, cz + t), (cx - w, cy - of, cz), (cx - w, cy - of, cz + t), (cx - w, cy + y, cz), (cx - w, cy + y, cz + t), (cx, cy + t, cz), (cx, cy + t, cz + t))
                        face_type = "pos"
                    elif abs(cur_ang) > r45 and abs(cur_ang - ang) < r45:  # step on corner
                        x = (ex / (abs(w_rot) - r45)) * (abs(cur_ang) - r45)
                        y2 = (gy / r45) * (abs(cur_ang - ang))
                        x -= t
                        temp_x = x
                        cx = hw
                        ...
                        v += ((cx, cy, cz), (cx, cy, cz + t))
                        face_type = "pos_tri"
                    else:  # last step
                        points = ((cx, cy), (cx - w + temp_x - t, cy + gy - t), (cx - w + temp_x - t, cy + gy), (cx - t, cy))
                        counter = 0
                        points += ((cx + of, cy), (cx - w + temp_x - t, cy + gy - of - t), (cx - w + temp_x - t, cy + gy + ex - temp_x - t), (cx - t, cy))
                        for i in points:
                            if counter in (1, 2, 5, 6):
                                origin = (cx - w + temp_x, cy + gy - t)
                            else:
                                origin = (cx, cy)
        ...
        if face_type == "normal":
            ...
            for i in range(2):
                f += ((p, p + 6, p + 7, p + 1), (p, p + 1, p + 3, p + 2), (p + 2, p + 3, p + 5, p + 4), (p + 4, p + 5, p + 7, p + 6), (p + 1, p + 7, p + 5, p + 3), (p, p + 2, p + 4, p + 6))
                p += 8
        elif face_type == "neg":
            for i in range(2):
                f += ((p, p + 1, p + 7, p + 6), (p, p + 2, p + 3, p + 1), (p + 2, p + 4, p + 5, p + 3), (p + 4, p + 6, p + 7, p + 5), (p + 1, p + 3, p + 5, p + 7), (p, p + 6, p + 4, p + 2))
                p += 8
...
def normal_stairs(self, context, tw, rh, of, os, w, overhang, steps, is_close, set_in, is_riser, is_light, landing, is_custom_tread):
    ...
    for jack in range(jack_num):  # each jack
        extra = 6 / 39.3701
        t_height = t_steps * riser
        t_depth = t_steps * tread - inch
        ...
        if is_close == False or (jack != 0 and jack != jack_num - 1):
            point = [extra + cy, 0.0]
            point_1 = [tread * t_steps + cy - inch, (riser * t_steps) - extra]
        ...
        last = 0.0
        out_faces = []
        if is_close == True and jack in (0, jack_num - 1):
            last = t_height - extra
        # stairs
        cz = riser
        sy = cy
        for stair in range(t_steps):
            ...
            if jack_type == "normal":
                ...
                if stair == 0:
                    ...
                    v += ((cx, cy + width - ty, cz), (cx + inch, cy + width - ty, cz), (cx + t, cy + width - ty, cz))
                    cy += tread + tread - ty
                    cz += riser + riser + t
                    if jack == 0:
                        face_type = "first"
                    else:
                        face_type = "first_right"
                ...
                if face_type == "first":
                    ...
                    if t_steps != 1:
                        f.append((p + 7, p + 10, p + 11, p + 8))
                    else:
                        f.append((p + 6, p + 7, p + 10, p + 9))
                    ...
                    if t_steps != 1:
                        f.append((p + 6, p + 9, p + 10, p + 7))
                    else:
                        f.append((p + 7, p + 8, p + 11, p + 10))
...
def StairsMaterials(self, context):
    ...
    if context.scene.render.engine == "CYCLES":
        ...
        if o.s_col_image == "":
            error = True
        if o.s_norm_image == "" and o.s_is_bump == True:
            error = True
        ...
        if error == False and len(o.data.materials) >= 1:
            ...
            if mat != None:
                ...
                o.data.materials[0].name = "stairs_" + o.name
            else:
        ...
        if o.s_style in ("1", "3"):
            ...
            if o.s_col_image2 == "":
                error2 = True
            if o.s_norm_image2 == "" and o.s_is_bump2 == True:
                error2 = True
            if error2 == False and len(o.data.materials) >= 2:
                ...
                if mat != None:
                    ...
                    if o.s_style == "1":
                        o.data.materials[1].name = "jacks_" + o.name
                    else:
                        o.data.materials[1].name = "pole_" + o.name
                else:
                    self.report({"ERROR"}, "Images Not Found, Make Sure Path Is Correct")
            else:
        ...
        if area.type == 'VIEW_3D':
            for space in area.spaces:
                if space.type == 'VIEW_3D':
                    if bpy.context.object.f_is_preview == True:
