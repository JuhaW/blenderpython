...
def create_siding(context, mat, if_tin, if_wood, if_vinyl, is_slope, ow, oh, bw, slope, is_width_vary, width_vary,
    ...
    if mat == "1" and if_wood == "1":  # Wood > Vertical
        ...
        for i in batten_pos[1]:
            ...
            if cur_x + baw < ow:
                p = len(v)
                v.append((cur_x, -0.02539, 0.0))
                v.append((cur_x, -0.04444, 0.0))  # Bottom > Left
                v.append((cur_x + baw, -0.04444, 0.0))
                v.append((cur_x + baw, -0.02539, 0.0))  # Bottom > Right
                v.append((cur_x, -0.02539, cur_z))
                v.append((cur_x, -0.04444, cur_z))  # Top > Left
                if is_slope == False:  # flat
                    v.append((cur_x + baw, -0.04444, oh))
                    v.append((cur_x + baw, -0.02539, oh))  # Top > Right
                elif is_slope == True:
                    if cur_x < ow / 2 and cur_x + baw < ow / 2:  # slope up
                        v.append((cur_x + baw, -0.04444, cur_z + z_dif))
                        v.append((cur_x + baw, -0.02539, cur_z + z_dif))  # Top > Right
                    elif cur_x < ow / 2 and cur_x + baw > ow / 2:  # middle board
                        del v[len(v) - 1]
                        del v[len(v) - 1]  # remove top > right set
                        v.insert(len(v) - 2, (ow / 2, -0.04444, 0.0))
                        v.insert(len(v) - 2, (ow / 2, -0.02539, 0.0))  # insert bottom middle set
                        z_pos = oh - ((slope * ((ow / 2) - cur_x)) / 12)  # figure out whats on left and calculate height
                        v.append((cur_x, -0.02539, z_pos))
                        v.append((cur_x, -0.04444, z_pos))  # Top > Left
                        v.append((ow / 2, -0.04444, oh))
                        v.append((ow / 2, -0.02539, oh))  # Top > Middle
                        z_pos = oh - ((slope * ((cur_x + baw) - (ow / 2))) / 12)  # figure out what on right and calculate height
                        v.append((cur_x + baw, -0.04444, z_pos))
                        v.append((cur_x + baw, -0.02539, z_pos))
    ...
    while cur_x < ow:
        ...
        while cur_z < height:
            ...
            for face in f:
                faces.append(face)
            p -= 18
            faces.append((p + 18, p + 19, p + 20, p + 21))
            faces.append((p + 18, p + 21, p + 22, p + 23))
        cur_x += 0.22859
    return (verts, faces)
...
def bricks(oh, ow, is_slope, slope, b_w, b_h, b_offset, gap, ran_offset, b_vary, faces, verts, is_corner, is_invert, is_left, is_right):  # bricks
    ...
    while cur_z < oh:
        ...
        if is_corner == True:
            if is_left == True:
                if is_invert == False and offset == False:
                    cur_x = -(b_w / 2)
                elif is_invert == True and offset == True:
                    cur_x = -(b_w / 2)
                else:
                    cur_x = 0.0
            if is_right == True:
        ...
        while cur_x < last_x:
            ...
            if is_corner == False:
                ...
                if cur_x == 0.0 and offset == True:
                    b_w2 = b_w * off
                elif cur_x == 0.0 and offset == False and ran_offset == True:
                    b_w2 = b_w * off
                ...
                if is_left == False and offset == True and cur_x == 0.0:
                    b_w2 = b_w / 2
                else:
                    b_w2 = b_w
                # get end brick correct
                if offset == False and is_invert == False and cur_x + b_w > ow:
                    b_w2 = ow - cur_x
                    add = 1
                elif offset == True and is_invert == False and cur_x + b_w > last_x:
                    b_w2 = last_x - cur_x
                    add = 0
                elif offset == True and is_invert == True and cur_x + b_w > ow:
                    b_w2 = ow - cur_x
                    add = 1
                elif offset == False and is_invert == True and cur_x + b_w > last_x:
                    b_w2 = last_x - cur_x
                    add = 0
                if cur_z + b_h > oh:
                    b_h = oh - cur_z
...
def bricks_mortar(oh, ow, m_d, is_slope, slope, is_corner, is_left, is_right, convert, b_w, m_gap, bricks, x_off):  # creates mortar object
    ...
    if bricks == True:
        ...
        y = m_d
    bw = round(b_w / 39.3701, 5)
    gap = round(m_gap / 39.3701, 5)
    ...
    for i in corner_data:
        ...
        if cur_z < oh:
            while cur_x < i[2]:
                ...
                if cur_x + b_h + gap > i[2]:
                    bh2 = i[2] - cur_x - gap

                v = ((cur_x, 0.0, cur_z), (cur_x, -d, cur_z), (cur_x + bh2, -d, cur_z), (cur_x + bh2, 0.0, cur_z))
                cur_z += bw2
                v += ((cur_x, 0.0, cur_z), (cur_x, -d, cur_z), (cur_x + bh2, -d, cur_z), (cur_x + bh2, 0.0, cur_z))
                cur_z -= bw2
                cur_x += gap + bh2
...
def stone_sizes(num, columns, grid, total):
    ...
    for i in range(5):
        if rows >= 0:
            row = grid[rows * columns: (rows * columns) + columns]
            pos = n + (rows * columns)
    ...
    if o[18] != False:
        p.append((n, o[18]))
    # 2X2
    if o[9] != False and o[10] != False and o[16] != False:
        p.append((o[9], o[10], o[16], n))
    if o[16] != False and o[23] != False and o[24] != False:
        p.append((o[16], n, o[23], o[24]))
    if o[10] != False and o[11] != False and o[18] != False:
        p.append((o[10], o[11], n, o[18]))
    if o[18] != False and o[24] != False and o[25] != False:
        p.append((n, o[18], o[24], o[25]))
    # 3X1
    if o[15] != False and o[16] != False:
        p.append((o[15], o[16], n))
    if o[16] != False and o[18] != False:
        p.append((o[16], n, o[18]))
    if o[18] != False and o[19] != False:
        p.append((n, o[18], o[19]))
    # 3X2
    if o[8] != False and o[9] != False and o[10] != False and o[15] != False and o[16] != False:
        p.append((o[8], o[9], o[10], o[15], o[16], n))
    if o[9] != False and o[10] != False and o[11] != False and o[16] != False and o[18] != False:
        p.append((o[9], o[10], o[11], o[16], n, o[18]))
    if o[10] != False and o[11] != False and o[12] != False and o[18] != False and o[19] != False:
        p.append((o[10], o[11], o[12], n, o[18], o[19]))
    if o[15] != False and o[16] != False and o[22] != False and o[23] != False and o[24] != False:
        p.append((o[15], o[16], n, o[22], o[23], o[24]))
    if o[16] != False and o[18] != False and o[23] != False and o[24] != False and o[25] != False:
        p.append((o[16], n, o[18], o[23], o[24], o[25]))
    if o[18] != False and o[19] != False and o[24] != False and o[25] != False and o[26] != False:
        p.append((n, o[18], o[19], o[24], o[25], o[26]))
    # 3X3
    # first row
    ...
    if o[0] != False and o[1] != False and o[2] != False and o[3] != False and o[7] != False and o[8] != False and o[9] != False and o[10] != False \
            and o[14] != False and o[15] != False and o[16] != False:
            p.append((o[0], o[1], o[2], o[3], o[7], o[8], o[9], o[10], o[14], o[15], o[16], n))
    if o[1] != False and o[2] != False and o[3] != False and o[4] != False and o[8] != False and o[9] != False and o[10] != False and o[11] != False \
            and o[15] != False and o[16] != False and o[18] != False:
            p.append((o[1], o[2], o[3], o[4], o[8], o[9], o[10], o[11], o[15], o[16], n, o[18]))
    if o[2] != False and o[3] != False and o[4] != False and o[5] != False and o[9] != False and o[10] != False and o[11] != False and o[12] != False \
            and o[16] != False and o[18] != False and o[19] != False:
            p.append((o[2], o[3], o[4], o[5], o[9], o[10], o[11], o[12], o[16], n, o[18], o[19]))
    if o[3] != False and o[4] != False and o[5] != False and o[6] != False and o[10] != False and o[11] != False and o[12] != False and o[13] != False \
            and o[18] != False and o[19] != False and o[20] != False:
            p.append((o[3], o[4], o[5], o[6], o[10], o[11], o[12], o[13], n, o[18], o[19], o[20]))
    # second row
    if o[7] != False and o[8] != False and o[9] != False and o[10] != False and o[14] != False and o[15] != False and o[16] != False and o[21] != False \
            and o[22] != False and o[23] != False and o[24] != False:
            p.append((o[7], o[8], o[9], o[10], o[14], o[15], o[16], n, o[21], o[22], o[23], o[24]))
    if o[8] != False and o[9] != False and o[10] != False and o[11] != False and o[15] != False and o[16] != False and o[18] != False and o[22] != False \
            and o[23] != False and o[24] != False and o[25] != False:
            p.append((o[8], o[9], o[10], o[11], o[15], o[16], n, o[18], o[22], o[23], o[24], o[25]))
    if o[9] != False and o[10] != False and o[11] != False and o[12] != False and o[16] != False and o[18] != False and o[19] != False and o[23] != False \
            and o[24] != False and o[25] != False and o[26] != False:
            p.append((o[9], o[10], o[11], o[12], o[16], n, o[18], o[19], o[23], o[24], o[25], o[26]))
    if o[10] != False and o[11] != False and o[12] != False and o[13] != False and o[18] != False and o[19] != False and o[20] != False and o[24] != False \
            and o[25] != False and o[26] != False and o[27] != False:
            p.append((o[10], o[11], o[12], o[13], n, o[18], o[19], o[20], o[24], o[25], o[26], o[27]))
    # third row
    if o[14] != False and o[15] != False and o[16] != False and o[21] != False and o[22] != False and o[23] != False and o[24] != False and o[28] != False \
            and o[29] != False and o[30] != False and o[31] != False:
            p.append((o[14], o[15], o[16], n, o[21], o[22], o[23], o[24], o[28], o[29], o[30], o[31]))
    if o[15] != False and o[16] != False and o[18] != False and o[22] != False and o[23] != False and o[24] != False and o[25] != False and o[29] != False \
            and o[30] != False and o[31] != False and o[32] != False:
            p.append((o[15], o[16], n, o[18], o[22], o[23], o[24], o[25], o[29], o[30], o[31], o[32]))
    if o[16] != False and o[18] != False and o[19] != False and o[23] != False and o[24] != False and o[25] != False and o[26] != False and o[30] != False \
            and o[31] != False and o[32] != False and o[33] != False:
            p.append((o[16], n, o[18], o[19], o[23], o[24], o[25], o[26], o[30], o[31], o[32], o[33]))
    if o[18] != False and o[19] != False and o[20] != False and o[24] != False and o[25] != False and o[26] != False and o[27] != False and o[31] != False \
            and o[32] != False and o[33] != False and o[34] != False:
            p.append((n, o[18], o[19], o[20], o[24], o[25], o[26], o[27], o[31], o[32], o[33], o[34]))
    return p
...
def stone_grid(oh, ow, avw, avh, sr):  # generate grid and figure stone sizes
    ...
    while clist != []:
        ...
        pick = []
        if twos != []:
            c = [pick.append("two") for i in range(int((-0.105 * rs) + 10))]
        if threes != []:
            c = [pick.append("three") for i in range(int((-0.11 * rs) + 10))]
        if fours != []:
            c = [pick.append("four") for i in range(int((-0.09 * rs) + 10))]
        if sixs != []:
            c = [pick.append("six") for i in range(int((-0.12 * rs) + 10))]
        if nines != []:
            c = [pick.append("nine") for i in range(int((-0.14 * rs) + 10))]
        if twelves != []:
            c = [pick.append("twelve") for i in range(int((-0.15 * rs) + 10))]
        if sr != 0:
...
def UpdateSiding(self, context):
    ...
    if o.object_add == "add":
        ...
        if o.cut_name == "none":  # if cutter object hasn't been created yet
            ...
            coords = [o.location.copy(), o.rotation_euler.copy()]
        else:
    ...
    if o.mat in ("5", "6") and o.object_add == "add":
        if o.is_slope == True:
            ...
            for obs in context.scene.objects:
                if obs.name.startswith("slope_cut"):
                    obs.select = True
                else:
                    obs.select = False
    ...
    if o.mat in ("5", "6"):
        if o.mat == "5":
            depth = round(o.m_depth, 5)
            bricks = True
        else:
            depth = round(o.s_mortar, 5)
            bricks = False
        ...
        bc_m = bpy.data.meshes.new("mortar")
        bc_m.from_pydata(verts3, [], faces3)
        mortar = bpy.data.objects.new("mortar", bc_m)
        context.scene.objects.link(mortar)
        mortar.location = o.location
        mortar.rotation_euler = o.rotation_euler
        mortar.scale = o.scale
        enter = True
        for i in mats:
            if "mortar_" in i or len(mats) >= 2:
                enter = False
        if enter == True:
        ...
        o.select = False
        mortar.select = True
        context.scene.objects.active = mortar
        bpy.ops.object.modifier_add(type="BEVEL")
    ...
    if o.mat == "3" and o.is_screws == True:
        ...
        screwOb.location = o.location
        screwOb.rotation_euler = o.rotation_euler
        for ob in context.scene.objects:
            ob.select = False
        screwOb.select = True
        o.select = True
    ...
    if o.mat in ("5", "6"):  # join mortar and brick
        ...
        if o.is_soldier == True and o.is_cutout == True:
            ...
            soldier.location = o.location.copy()
            soldier.rotation_euler = o.rotation_euler.copy()
            soldier.scale = o.scale.copy()
            soldier.select = True
            o.select = True
            bpy.context.scene.objects.active = o
            bpy.ops.object.join()
...
def SidingMaterial(self, context):
    ...
    if o.mat in ("2", "3", "4"):  # vinyl, tin, fiber cement
        ...
        if o.col_image == "":
            ...
            error = True
        elif o.is_bump == True and o.norm_image == "":
        ...
        if error == False:
            ...
            if mat != None:
                if o.mat == "6" and len(o.data.materials) >= 2:
                    ...
                    error = True
                    self.report({"ERROR"}, "Material Needed For Stone Not Found")
            else:
        ...
        if len(o.data.materials) >= 2:
            ...
            self.report({"ERROR"}, "Material Needed For Bricks//Stones Not Found, Please Update Siding")
            error = True
    if o.mat in ("5", "6"):  # mortar
        if len(o.data.materials) >= 2:
            if "mortar_temp" in o.data.materials:
                mname = "mortar_temp"
            else:
                mname = o.data.materials[1].name
            mat2 = Mortar(bpy, context, o.mortar_color, o.mortar_bump, mname)
            o.data.materials[1] = mat2.copy()
            o.data.materials[1].name = "mortar_" + o.name
        else:
            self.report({"ERROR"}, "Material Needed For Mortar Not Found, Please Update Siding")
            error = True
    # set material
    if cur_mat != None and error == False and o.mat != "5":  # update current material
        cur_mat = mat.copy()
        o.data.materials[index] = cur_mat
        ...
        if area.type == 'VIEW_3D':
            for space in area.spaces:
                if space.type == 'VIEW_3D':
                    if bpy.context.object.is_preview == True:
    ...
    def execute(self, context):
        ...
        if o.object_add == "convert" and o.cut_name in bpy.data.objects:
            ...
            for i in layers:
                ...
                counter += 1
            o_layers = []
            for i in bpy.context.object.layers:
                o_layers.append(i)
            for area in bpy.context.screen.areas:
                if area.type == 'VIEW_3D':
                    override = bpy.context.copy()
