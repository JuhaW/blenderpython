...
# refresh surfaces
        if bpy.context.scene.get('surfrestrictor') is None:
            print("поверхности не фильтруются")
            surf = 'OUTLINER_OB_SURFACE'
            for ob in bpy.context.scene.objects:
                if ob.type == 'SURFACE':
                    if ob.get('ignore_restrictors') is None:
...
def register():
    ...
    bpy.utils.register_class(RestrictorShow)
    bpy.utils.register_class(IgnoreRestrictors)
    bpy.utils.register_class(RestrictorSelection)
    bpy.utils.register_class(RestrictorMesh)
    bpy.utils.register_class(RestrictorCurve)
    bpy.utils.register_class(RestrictorArm)
    bpy.utils.register_class(RestrictorEmpty)
    bpy.utils.register_class(RestrictorCam)
    bpy.utils.register_class(RestrictorLamp)
    bpy.utils.register_class(RestrictorLat)
    bpy.utils.register_class(RestrictorFont)
    bpy.utils.register_class(RestrictorMeta)
    bpy.utils.register_class(RestrictorSurf)
    bpy.utils.register_class(RestrictorSound)
    bpy.utils.register_class(RefreshRestrictors)


def unregister():
    bpy.types.VIEW3D_HT_header.remove(view3d_select_menu)
    bpy.utils.unregister_class(RestrictorShow)
    bpy.utils.unregister_class(IgnoreRestrictors)
    bpy.utils.unregister_class(RestrictorSelection)
    bpy.utils.unregister_class(RestrictorMesh)
    bpy.utils.unregister_class(RestrictorCurve)
    bpy.utils.unregister_class(RestrictorArm)
    bpy.utils.unregister_class(RestrictorEmpty)
    bpy.utils.unregister_class(RestrictorCam)
    bpy.utils.unregister_class(RestrictorLamp)
    bpy.utils.unregister_class(RestrictorLat)
    bpy.utils.unregister_class(RestrictorFont)
    bpy.utils.unregister_class(RestrictorMeta)
    bpy.utils.unregister_class(RestrictorSurf)
    bpy.utils.unregister_class(RestrictorSound)
