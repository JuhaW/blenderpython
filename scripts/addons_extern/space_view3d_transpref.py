...
class TranspRefPanel(bpy.types.Panel):
	...
	def draw(self, context):
		global cbon, regionui, region3d, start
		if not(cbon):
			cbon = 1
			for region in bpy.context.area.regions:
				if region.type == "UI":
			...
			for region in bpy.context.area.regions:
				if region.type == "WINDOW":
					region3d = region
					self._handle = region.callback_add(redraw, (), 'POST_PIXEL')
		scn = context.scene
		if waiting:
...
for region in bpy.context.area.regions:
	if region.type == "WINDOW":
		_handle = region.callback_add(redraw, (), 'POST_PIXEL')
