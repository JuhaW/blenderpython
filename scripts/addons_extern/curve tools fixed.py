...
class ctoolsprops(bpy.types.PropertyGroup):
    dodriver = BoolProperty(name="dodriver", default=False)
    # selection = CollectionProperty(name="Selection")
    steps = IntProperty(name="steps", default=12, min=2)
    spans = IntProperty(name="spans", default=12, min=2)
    intype = EnumProperty(name="intype", items=myitems, default='3')
    tension = FloatProperty(name="tension", min=-3.0, max=1.0, default=0.0)
    bias = FloatProperty(name="bias", min=-1.0, max=1.0, default=0.0)
    proportional = BoolProperty(name="proportional", default=False)
    clen = FloatProperty(name="clen", default=0.0, precision=5)
    limit = FloatProperty(name="limit", default=0.1, precision=3)


class driversdata(bpy.types.PropertyGroup):
    objname = None
    curves = None

bpy.utils.register_class(ctoolsprops)
bpy.utils.register_class(driversdata)

bpy.types.Scene.driversdata = CollectionProperty(type=driversdata)

bpy.types.Scene.ctools = PointerProperty(type=ctoolsprops)

# bpy.utils.register_class(sprops)

bpy.types.Scene.dodriver = BoolProperty(name="dodriver")
...
bpy.types.Scene.tension = FloatProperty(name="tension", min=-3.0, max=1.0, default=0.0)
bpy.types.Scene.bias = FloatProperty(name="bias", min=-1.0, max=1.0, default=0.0)
...
class ArcLengthOperator(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        context.scene.ctools.clen = arclength(objs)
        return {'FINISHED'}
...
def loftdriver(name, objs, intype):
    ...
    for i, l in enumerate(objs):
        ...
        in_edit = in_edit or objs[i].mode == 'EDIT'
    ...
    bm = dic.setdefault(me.name, driven_mesh(me))
    if not bm.is_valid:
        dic.clear()
        return -1
...
def loft(objs, steps, spans, interpolation=1, tension=0.0, bias=0.0):
    verts = []
    # print(steps, spans)
...
class LoftOperator(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        objs = bpy.selection
        ctools = context.scene.ctools
        spans = ctools.spans
        steps = ctools.steps

        tension = ctools.tension
        bias = ctools.bias

        intype = int(ctools.intype)

        verts = loft(objs, steps, spans, intype, tension, bias)
        ...
        if context.scene.ctools.dodriver:
...
def birail1driver(name, objs):
    ...
    try:
        ...
        obj = bpy.context.object
        # print(obj)
        if obj.type == "CURVE" and obj.animation_data != None:
            for drv in obj.animation_data.drivers:
                # print(drv.data_path)
                if drv.data_path == '["driver"]':
                    obj.driver_remove('["driver"]')
        return False
...
class Birail1Operator(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        scn = context.scene
        spans = scn.ctools.spans
        steps = scn.ctools.steps
        ...
        if verts != []:
            ...
            newobj['spans'] = spans
            newobj['prop'] = prop
            # print("newobj", newobj.keys())
            if scn.ctools.dodriver:
...
class MergeSplinesOperator(bpy.types.Operator):
    '''Merge spline points inside a limit'''
...
class CurvePanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        align = True
        row = layout.row()
        row.prop(ctools, "intype", text="")
        row.prop(ctools, "dodriver", text="Driven")
        if ctools.intype == '3':  # Hermite interp
            row = layout.row(align=align)
            row.prop(ctools, "tension")
            row.prop(ctools, "bias")

        row = layout.row(align=align)
        row.prop(ctools, "steps")
        row.prop(ctools, "spans")
        row = layout.row(align=align)
        row.operator("mesh.update_fix", text="Update Fix")
        row.operator("mesh.loft_operator", text="Loft")

        row = layout.row(align=align)
        row.prop(ctools, "proportional", text="Proportional")
        row.operator("mesh.birail1_operator", text="Birail 1")

        row = layout.row(align=align)

        row.label(text=str(ctools.clen))
        row.operator("curve.arc_length_operator", text="Calc Length")

        row = layout.row(align=align)
        row.prop(ctools, "limit", text="Limit")
