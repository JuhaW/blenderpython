...
def isPartOfFlareControler(object, flareControler):
    if object is None or flareControler is None:
        return False
...
def getCorrespondingFlareControler(object):
    ...
    if hasLinkToFlareControler(object):
        return bpy.data.objects.get(object[linkToFlareControlerPropertyName])
...
def getCorrespondingElement(object):
    if hasFlareElementAttribute(object):
        return bpy.data.objects.get(object[elementNamePropertyName])
...
def getActiveElement():
    ...
    if isPartOfFlareControler(activeElement, activeFlare):
...
class LensFlaresPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        activeFlare = getActiveFlare()
        if len(flares) == 0:
            layout.label("no flares in this scene", icon="INFO")
        else:
...
class LensFlareSettingsPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        box = layout.box()
        if len(elements) == 0:
            box.label("no elements on this flare", icon="INFO")
        else:
            ...
            for element in elements:
                ...
                if element == activeElement:
                    ...
                    selectElement = row.operator("lens_flares.select_flare_element", text=element[elementNamePropertyName])
