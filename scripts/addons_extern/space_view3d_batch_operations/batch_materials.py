...
class BatchOperations:
    ...
    def to_material(cls, material):
        if isinstance(material, Material):
            return material
        if isinstance(material, MaterialSlot):
    ...
    def icon_kwargs(cls, idname, use_value=True):
        ...
        if (not idname) or (not use_value):
    ...
    def merge_identical(cls):
        ...
        def compare_node_tree(rna_prop, valueA, valueB):
            ...
            if (valueA is None) or (valueB is None):
                return False
            if not BlRna.compare(valueA.animation_data, valueB.animation_data):
                return False
