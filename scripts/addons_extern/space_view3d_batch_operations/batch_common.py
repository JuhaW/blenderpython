...
def apply_shapekeys(obj):
    ...
    if obj.data.users > 1:
        obj.data = obj.data.copy()  # don't affect other objects
...
def apply_modifiers(objects, scene, idnames, options=(), apply_as='DATA'):
    ...
    for obj in objects:
        ...
        if apply_shape_keys:
            apply_shapekeys(obj)  # also makes single-user
        ...
        for md in tuple(obj.modifiers):
            if (idnames is not None) and (md.type not in idnames):
                continue
    ...
    if active_obj in objects_to_delete:
        active_obj = None
...
class Operator_batch_repeat_actions:
    ...
    def invoke(self, context, event):
        ...
        for i, report in enumerate(reversed(reports)):
            ...
            if len(cls.compiled) >= cls.max_shown_actions:
                break
    ...
    def execute(self, context):
        ...
        if not any(item.value for item in self.operations):
            return {'CANCELLED'}
...
class Operator_batch_streamline_meshes:
    ...
    def apply(self, context, select_all=True):
        ...
        if self.sort:
            sort_elements = set()
            if self.sort_verts:
                sort_elements.add('VERT')
            if self.sort_edges:
                sort_elements.add('EDGE')
            if self.sort_faces:
                sort_elements.add('FACE')
        ...
        if active_obj and (active_obj.type == 'MESH') and (active_obj.mode == 'EDIT'):
            ...
            if self.apply_modifiers:
                ...
                if self.convert_to_mesh:
                    apply_modifiers_options.add('CONVERT_TO_MESH')
                if self.apply_modifiers_make_single_user:
                    apply_modifiers_options.add('MAKE_SINGLE_USER')
                if self.apply_modifiers_remove_disabled:
                    apply_modifiers_options.add('REMOVE_DISABLED')
                if self.apply_modifiers_delete_operands:
                    apply_modifiers_options.add('DELETE_OPERANDS')
                if self.apply_modifiers_shape_keys:
                    apply_modifiers_options.add('APPLY_SHAPE_KEYS')
            ...
            for obj in IndividuallyActiveSelected(objects, make_visble=True):
                if obj.type not in BlEnums.object_types_geometry:
                    continue
...
def Operator_Hide(self, context, event, idnames="", state=False):
    if event is not None:
        if event.shift:
            state = False  # Shift -> force show
        elif event.ctrl:
            state = True  # Ctrl -> force hide
...
@addon.Operator(idname="object.batch_parent_to_empty", options={'INTERNAL', 'REGISTER'}, label="Parent To Empty", description="Click: Parent To Empty (+Ctrl: place parent at 3D cursor)")
...
def make_category(globalvars, idname_attr="name", **kwargs):
    ...
    @addon.Menu(idname="OBJECT_MT_batch_{}_add".format(category_name), description="Add {}(s)".format(Category_Name))
    ...
    @addon.Operator(idname="view3d.pick_{}".format(category_name_plural), options={'INTERNAL', 'REGISTER'}, description="Pick {}(s) from the object under mouse".format(Category_Name))
    ...
    @addon.Operator(idname="object.batch_{}_copy".format(category_name), options={'INTERNAL'}, description="Click: Copy")
    ...
    @addon.Operator(idname="object.batch_{}_paste".format(category_name), options={'INTERNAL', 'REGISTER'}, description="Click: Paste (+Ctrl: Override, +Shift: Add, +Alt: Filter)")
    ...
    @addon.Operator(idname="object.batch_{}_add".format(category_name), options={'INTERNAL', 'REGISTER'}, description="Click: Add")
    def Operator_Add(self, context, event, idnames="", create=False):
        ...
        if is_ID and create:
            idnames = BatchOperations.new(Category_Name)
        BatchOperations.add(options.iterate_objects(context), idnames)
        category.tag_refresh()
        return {'FINISHED'}

    @addon.Operator(idname="object.batch_{}_assign".format(category_name), options={'INTERNAL', 'REGISTER'}, description="Click: Assign menu, Alt+Click: Alt-Assign action, Shift+Click: Shift-Assign action".format(category_name))
    ...
    @addon.Operator(idname="object.batch_{}_assign_action".format(category_name), options={'INTERNAL', 'REGISTER'}, description="Click: Perform the corresponding action (+Ctrl: globally)".format(category_name))
    ...
    @addon.Operator(idname="object.batch_{}_name".format(category_name), options={'INTERNAL', 'REGISTER'}, description="Click: Select objects, Shift+Click: (De)select row (+Ctrl: set other rows), Ctrl+Click: Rename, Alt+Click: Assign action (+Ctrl: globally)")
    def Operator_Name(self, context, event, idnames="", index=0, title=""):
        ...
        if event.alt:
            ...
            if options.synchronize_selection:
                ...
                if toggled_state:
                    BatchOperations.select(context, idnames, 'AND!')
        elif event.ctrl:
    ...
    @addon.Operator(idname="object.batch_{}_remove".format(category_name), options={'INTERNAL', 'REGISTER'}, description="Click: Remove (+Ctrl: globally), Alt+Click: Purge")
        ...
        def iterate(self, context=None, globally=False, selected=None, search_in=None):
            if search_in is None:
                if not context:
                    context = bpy.context
                if selected is None:
                    selected = self.prioritize_selection
    ...
    class AggregateInfo:
        ...
        def collect_info(cls, items, count_users=False):
            ...
            for obj, idname in BatchOperations.iter_scene_objs_idnames(bpy.context.scene):
                if idname not in infos:
                    continue  # ignore object not in Filter
        ...
        def extract_info(cls, infos, item, idname=None, count_users=False):
            if idname is None:
                idname = getattr(item, cls.idname_attr)
    ...
    for cmd, cmd_kwargs in _nongeneric_actions:
        ...
        if cmd == "aggregate_toggle":
            ...
            if icons is None:
                icons = ('CHECKBOX_HLT', 'CHECKBOX_DEHLT')
    ...
    for name, params in aggregate_attrs:
        ...
        if aggr is None:
            aggr = dict(init=('BOOL', {"same", "min", "max", "mean"}), fallback=False, invert=invert)
    ...
    def draw_toggle_or_action(layout, item, item_idnames, title, icon_novalue, cmd, cmd_kwargs, from_obj, emboss=True):
        if cmd == "aggregate_toggle":
            ...
            if from_obj:
                ...
                if hasattr(op, "category_idnames"):
                    op.category_idnames = item_idnames
            else:
        ...
        def draw(self, layout):
            ...
            with layout.column(True):
                for item in self.items:
                    ...
                    if not self.is_anything_selected:
                        can_affect = True
                    ...
                    with layout.row(True)(active=not is_excluded):
                        ...
                        for cmd, cmd_kwargs, action_idname, from_obj, use_affect, after_name in nongeneric_actions_no_text:
                            if after_name:
                                continue
                            if action_idname not in options.quick_access:
                                continue
    ...
    if is_ID:
        @addon.Operator(idname="object.batch_{}_purge_unused".format(category_name), options={'INTERNAL', 'REGISTER'}, description="Click: Purge unused (+Ctrl: even those with use_fake_users)", label="Purge unused")
        ...
        @addon.Operator(idname="object.batch_{}_merge_identical".format(category_name), options={'INTERNAL', 'REGISTER'}, description="Click: Merge identical", label="Merge identical")
    ...
    @addon.Operator(idname="object.batch_{}_refresh".format(category_name), options={'INTERNAL', 'REGISTER'}, description="Click: Force refresh, Ctrl+Click: Toggle auto-refresh")
