...
class BakePass(bpy.types.PropertyGroup):
    ...
    def get_cycles_pass_type(self):
        ...
        if self.pass_name == "UV":
            return "UV"
        if self.pass_name == "EMIT":
            return "EMIT"
        if self.pass_name == "ENVIRONMENT":
            return "ENVIRONMENT"
        if self.pass_name == "DIFFUSE_DIRECT":
            return "DIFFUSE_DIRECT"
        if self.pass_name == "DIFFUSE_INDIRECT":
            return "DIFFUSE_INDIRECT"
        if self.pass_name == "DIFFUSE_COLOR":
            return "DIFFUSE_COLOR"
        if self.pass_name == "GLOSSY_DIRECT":
            return "GLOSSY_DIRECT"
        if self.pass_name == "GLOSSY_INDIRECT":
            return "GLOSSY_INDIRECT"
        if self.pass_name == "GLOSSY_COLOR":
            return "GLOSSY_COLOR"
        if self.pass_name == "TRANSMISSION_DIRECT":
            return "TRANSMISSION_DIRECT"
        if self.pass_name == "TRANSMISSION_INDIRECT":
            return "TRANSMISSION_INDIRECT"
        if self.pass_name == "TRANSMISSION_COLOR":
            return "TRANSMISSION_COLOR"
        if self.pass_name == "SUBSURFACE_DIRECT":
            return "SUBSURFACE_DIRECT"
        if self.pass_name == "SUBSURFACE_INDIRECT":
            return "SUBSURFACE_INDIRECT"
        if self.pass_name == "SUBSURFACE_COLOR":
...
class BakeJob(bpy.types.PropertyGroup):
    ...
    def get_render_resolution(self):
        ...
        if aa == '4':
            return [self.resolution_x * 4, self.resolution_y * 4]
...
class MeltdownBakeOp(bpy.types.Operator):
    ...
    def cleanup_render_target(self):
        bakeImage = bpy.data.images["MDtarget"]

        # call compo trees here
        self.compo_nodes_margin(bakeImage)
    ...
    def make_duplicates_real(self, object, oldMatrix, is_parentHipolyMember):
        mds = bpy.context.scene.meltdown_settings
        pair = mds.bake_job_queue[self.job].bake_queue[self.pair]
        ...
        if object.type == 'EMPTY' and object.dupli_group:
            for dupObj in object.dupli_group.objects:
                if dupObj.type == 'EMPTY' and dupObj.dupli_group:
                    if is_parentHipolyMember:
                        self.make_duplicates_real(dupObj, new_matrix, True)
                    else:
                        self.make_duplicates_real(dupObj, new_matrix, False)
                else:
                    copyDupObj = dupObj.copy()
                    copyDupObj.name += "_MD_TMP"
                    bpy.data.scenes["MD_TEMP"].objects.link(copyDupObj)
                    if is_parentHipolyMember:  # if parent was in hipoly group/obj then link child to hipoly to
                        if pair.hp_obj_vs_group == "GRP":
                            bpy.data.groups[pair.highpoly + "_MD_TMP"].objects.link(copyDupObj)
                        else:
                            bpy.data.groups["NoGroup" + pair.highpoly + "_MD_TMP"].objects.link(copyDupObj)
                    copyDupObj.matrix_world = new_matrix * copyDupObj.matrix_world

            # bpy.data.scenes["MD_TEMP"].objects.unlink(object) #remove empty from baking
    ...
    def scene_copy(self):
        ...
        if pair.highpoly != "":
            if pair.hp_obj_vs_group == "GRP":
                ...
                bpy.data.groups.new("NoGroup" + pair.highpoly + "_MD_TMP")  # create group with obj name (but how can I be sure it was not renamed...
                bpy.data.groups["NoGroup" + pair.highpoly + "_MD_TMP"].objects.link(bpy.data.scenes["MD_TEMP"].objects[pair.highpoly + "_MD_TMP"])
        for object in bpy.data.scenes["MD_TEMP"].objects:  # make dupli instances real
            if object.type == 'EMPTY' and object.dupli_group:
                if pair.highpoly != "":  # link obj's from group pro to hipoly group if obj is member of hipoly bake group
                    if pair.hp_obj_vs_group == "GRP":
                        if any(groupObj == object for groupObj in bpy.data.groups[pair.highpoly + "_MD_TMP"].objects):
                            self.make_duplicates_real(object, Matrix.Identity(4), True)  # true  = obj is in baking group (so childs should too)
                        else:
                            self.make_duplicates_real(object, Matrix.Identity(4), False)
                    else:
                        # __import__('code').interact(
                        #     local={k: v for ns in (globals(), locals()) for k, v in ns.items()})
                        if any(groupObj == object for groupObj in bpy.data.groups["NoGroup" + pair.highpoly + "_MD_TMP"].objects):
                            self.make_duplicates_real(object, Matrix.Identity(4), True)  # true  = obj is in baking group (so childs should too)
                        else:
                            self.make_duplicates_real(object, Matrix.Identity(4), False)
                bpy.data.scenes["MD_TEMP"].objects.unlink(object)  # remove empty from baking
    ...
    def pass_material_id_prep(self):
        ...
        if pair.highpoly != "":
            if pair.hp_obj_vs_group == "GRP":
                ...
                for object in bpy.data.groups["NoGroup" + pair.highpoly + "_MD_TMP"].objects:
                    change_material(object)
    ...
    def prepare_scene(self):
        ...
        # all hipoly pairs  visibility
        if bakepass.environment_highpoly == True:  # bake all hipoly object in  current pass
        ...
        #     pass
        # if bakepass.environment_highpoly == False:
        #     pass
    ...
    def bake_set(self):
        ...
        bpy.ops.object.bake(type=bpy.context.scene.cycles.bake_type, filepath="",
                            width=bj.get_render_resolution()[0], height=bj.get_render_resolution()[1], margin=bakeMargin,
    ...
    def remove_object(self, object):
        if bpy.data.objects[object.name]:
            if object.type == "MESH":
                ...
                bpy.data.objects.remove(object)
                try:  # if multi user skip removing mesh
    ...
    def compo_nodes_margin(self, targetimage):
        ...
        targetimage.scale(bj.resolution_x, bj.resolution_y)
        tree.nodes["Image"].image = targetimage

        bpy.ops.render.render(write_still=True, scene="MD_COMPO")
        aa = bj.antialiasing  # revert image size to original size for next bake
        if aa == '2':
            targetimage.scale(bj.resolution_x * 2, bj.resolution_y * 2)
        if aa == '4':
            targetimage.scale(bj.resolution_x * 4, bj.resolution_y * 4)
        bpy.ops.scene.delete()
    ...
    def is_empty_mat(self, context):
        ...
        for i_pair, pair in enumerate(bj.bake_queue):
            if pair.activated == True:
                pair = mds.bake_job_queue[self.job].bake_queue[i_pair]
                # pair_list = mds.bake_job_queue[self.job].bake_queue
                if pair.highpoly != "":
                    if pair.hp_obj_vs_group == "GRP":
    ...
    def execute(self, context):
        ...
        print("Total baking time: " + str(datetime.now() - TotalTime))
                ...
                for pass_i, bakepass in enumerate(bj.bake_pass_queue):
                    ...
                    if len(bakepass.props()) > 0:
                        ...
                        if "clean_environment" in bakepass.props():
                            subrow = box.row(align=True)
                            subrow.alignment = 'EXPAND'
                            subrow.prop(bakepass, 'environment_highpoly', text="Bake all Highpoly")
