...
class SmdExporter(bpy.types.Operator, Logger):
        ...
        try:
            ...
            if self.export_scene:
                ...
                if self.group == "":
                    ...
                    if group.vs.mute:
                        self.error(get_id("exporter_err_groupmuted", True).format(group.name))
                    elif not group.objects:
                        self.error(get_id("exporter_err_groupempty", True).format(group.name))
                    else:
                        self.exportId(context, group)
    ...
    def exportId(self, context, id):
        ...
        def find_basis_metaball(id):
            ...
            for meta in [ob for ob in bpy.data.objects if ob.type == 'META']:
                ns = meta.name.rsplit(".")
                if len(ns) == 1 or ns[0] != basis_ns[0]:
                    continue
                try:
                    ...
                    for vert in baked.data.vertices:
                        if balance_width == 0:
                            ...
                            if balance == 1:
                                ...
                                result.balance_vg.add([vert.index], balance, 'REPLACE')
    ...
    def writeSMD(self, id, bake_results, name, filepath, filetype='smd'):
        ...
        if not self.armature:
            ...
            if filetype == 'smd':
                print("- Exported", num_bones, "bones")
        ...
        if filetype == 'smd':
            ...
            for bake in [bake for bake in bake_results if bake.object.type != 'ARMATURE']:
                ...
                for poly in data.polygons:
                    if p % 10 == 0:
                        bpy.context.window_manager.progress_update(p / len(data.polygons))
    ...
    def writeDMX(self, id, bake_results, name, dir_path):
        ...
        def writeBone(bone):
            ...
            if not bone:
                ...
                cur_p = bone.parent
                while cur_p and not cur_p in self.exportable_bones:
                    cur_p = cur_p.parent
                if cur_p:
